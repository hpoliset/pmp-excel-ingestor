...
/**
 * This is a helper class to validate the token and mandatory fields and other
 * validations regarding program fields.
 * 
...
public class CreateEventHelper {
	...
	public Map<String, String> checkPartcicipantMandatoryFields(ParticipantRequest participant) {
		...
		if(null != participant.getEmail()){
			if (!participant.getEmail().matches("^[_A-Za-z0-9-]+(\\.[_A-Za-z0-9-]+)*@[A-Za-z0-9]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$")) {
	...
	public Map<String, String> checkUpdateEventAdminMandatoryFields(EventAdminChangeRequest eventAdminChangeRequest) {
		...
		if(null == eventAdminChangeRequest.getEventId() || eventAdminChangeRequest.getEventId().isEmpty() ){
			errors.put("eventId", "event Id is required");
		}else if(null != eventAdminChangeRequest.getEventId() &&  !eventAdminChangeRequest.getEventId().matches("^E[0-9]{6}$")){
			errors.put("eventId", "event Id invalid");
		}else if(0 == programService.getProgramIdByEventId(eventAdminChangeRequest.getEventId())){
			errors.put("eventId", "Invalid EventId - No event exists for the given event Id");
		}
		if(null == eventAdminChangeRequest.getNewCoordinatorEmail() || eventAdminChangeRequest.getNewCoordinatorEmail().isEmpty()){
	...
	/**
	 * Method is used to validate the mandatory parameters before persisting an event.
	 * Other validations include date validation and mobile number validation.
	 * @param program is validated to check all the mandatory parameters.
	 ...
	 * @param token need to be validated against mysrcm api.
	 * @throws HttpClientErrorException if client exception occurs.
	 * @throws JsonParseException while parsing json data.
