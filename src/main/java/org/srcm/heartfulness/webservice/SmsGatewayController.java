...
public class SmsGatewayController {
	...
	public String receiveSMS(@RequestParam(value = "phonecode", required = false) String phonecode,
			...
			@RequestParam(value = "location", required = false) String phoneno,
		...
		LOGGER.debug("Request param Method ", request.getMethod());
		Enumeration<String> parameterEnumeration = request.getParameterNames();
		while (parameterEnumeration.hasMoreElements()) {
			LOGGER.debug("Paramerter {}", parameterEnumeration.nextElement());
		}
		return "success";
	}
}
