	...
	public ResponseEntity<?> getEventDetails(@RequestHeader(value = "Authorization") String token,
		...
		try {
			...
			if (null == event.getAutoGeneratedEventId() || event.getAutoGeneratedEventId().isEmpty()) {
				errors.put("eventId", "Event Id cannot be empty");
				...
				if (programId > 0) {
					eventdetails = programService.getEventDetails(/*event.getAutoGeneratedEventId()*/emailList,user.getRole(),event.getAutoGeneratedEventId());
			...
			for (Event event : events) {
				...
				if (null == event.getAutoGeneratedEventId() || event.getAutoGeneratedEventId().isEmpty()) {
					...
					if (programId > 0) {
						...
						eventList.addAll(programService.createOrUpdateEvent(eventsList, accessLog.getId()));
					} else {
						errors.put("eventId", "Event ID does not exists");
						event.setErrors(errors);
						event.setStatus(ErrorConstants.STATUS_FAILED);
