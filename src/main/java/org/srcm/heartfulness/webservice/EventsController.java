		...
		try {
			...
			if (null == event.getAutoGeneratedEventId() || event.getAutoGeneratedEventId().isEmpty()) {
				...
				if (programId > 0) {
					eventdetails = programService.getEventDetails(event.getAutoGeneratedEventId());
					if(!isAdmin){
						if(!userProfile.getEmail().equals(eventdetails.getCoordinatorEmail())){
							accessLog.setErrorMessage("Logged in user :"+userProfile.getEmail() + "is trying to access event :"+event.getAutoGeneratedEventId() 
									+ "but event coordinator is "+eventdetails.getCoordinatorEmail());
							errors.put("eventId", "Access denied to view this event");
							eventdetails.setErrors(errors);
							eventdetails.setStatus(ErrorConstants.STATUS_FAILED);
							accessLog.setStatus(ErrorConstants.STATUS_ERROR);
						}else{
							eventdetails.setStatus(ErrorConstants.STATUS_SUCCESS);
							accessLog.setStatus(ErrorConstants.STATUS_SUCCESS);
						}
					}
				} else {
