...
import org.srcm.heartfulness.enumeration.ProgramStatus;
...
public class EventsController {
		...
		if (null == event.getAutoGeneratedEventId() || event.getAutoGeneratedEventId().isEmpty()) {
			...
			try{
				...
				if (programId > 0) {
					...
					eventdetails = programService.getEventDetails(emailList,user.getRole(),event.getAutoGeneratedEventId(),authToken,accessLog);
	...
	public ResponseEntity<?> getEventList(@RequestHeader(value = "Authorization") String authToken,
		...
		try{
			...
			//eventPagination.setTotalCount(programService.getProgramCountForLogInCoordinator(emailList, user.getRole(), authToken, accessLog));
			...
			eventList = programService.getEventListByEmailAndRole(emailList, user.getRole(), offset,authToken,accessLog,eventPagination);
	...
	public ResponseEntity<?> searchEvents(@RequestHeader(value = "Authorization") String authToken,
		...
		ErrorResponse srchParamResponse = null;
		if (!error.isEmpty()) {
			srchParamResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED, error);
			updatePMPAPIAccessLog(accessLog,ErrorConstants.STATUS_FAILED,error, StackTraceUtils.convertPojoToJson(srchParamResponse));
			return new ResponseEntity<ErrorResponse>(srchParamResponse, HttpStatus.PRECONDITION_FAILED);
		}
		String searchParamErrors = eventDashboardValidator.validateSearchParameters(searchRequest);
		if(!searchParamErrors.isEmpty()){
			srchParamResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED, searchParamErrors);
			updatePMPAPIAccessLog(accessLog,ErrorConstants.STATUS_FAILED,searchParamErrors, StackTraceUtils.convertPojoToJson(srchParamResponse));
			return new ResponseEntity<ErrorResponse>(srchParamResponse, HttpStatus.PRECONDITION_FAILED);
		}
		...
		try{
			...
			searchRequest.setEventList(programService.searchEventsWithUserRoleAndEmailId(searchRequest,emailList,user.getRole(), offset,authToken,accessLog,eventPgntn));
			searchRequest.setTotalCount(eventPgntn.getTotalCount());
			accessLog.setStatus(ErrorConstants.STATUS_SUCCESS);
		} catch(Exception ex){
	...
	public ResponseEntity<?> saveEvent(@RequestBody Event event,
		...
		if (null == event.getAutoGeneratedEventId() || event.getAutoGeneratedEventId().isEmpty()) {
			...
			int eventId = programService.getProgramIdByEventId(event.getAutoGeneratedEventId());
			if (eventId == 0) {
				ErrorResponse eResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED,
	...
	@RequestMapping(value = "/fetcheventdetails",
			method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_JSON_VALUE, 
			produces = MediaType.APPLICATION_JSON_VALUE)

	public ResponseEntity<?> getEventDetailsWithDecryptedID(@RequestHeader(value = "Authorization") String authToken,
			@RequestBody Event event, @Context HttpServletRequest httpRequest) {
		...
		Map<String, String> errors = new HashMap<String, String>();
		if (null == event.getAutoGeneratedEventId() || event.getAutoGeneratedEventId().isEmpty()) {
			errors.put("eventId", ErrorConstants.EMPTY_EVENT_ID);
			eventdetails.setErrors(errors);
			eventdetails.setStatus(ErrorConstants.STATUS_FAILED);
		} else {
			int programId = programService.getProgramIdByEventId(aesEncryptDecrypt.decrypt(event.getAutoGeneratedEventId(), env.getProperty(PMPConstants.SECURITY_TOKEN_KEY)));

			if (programId > 0) {
				accessLog.setStatus(ErrorConstants.STATUS_SUCCESS);
				eventdetails = programService.getEventDetails(emailList,user.getRole(),aesEncryptDecrypt.decrypt(
						event.getAutoGeneratedEventId(), env.getProperty(PMPConstants.SECURITY_TOKEN_KEY)),authToken,accessLog);
			} else {
				errors.put("eventId", ErrorConstants.INVALID_EVENT_ID);
				eventdetails.setErrors(errors);
				eventdetails.setStatus(ErrorConstants.STATUS_FAILED);
			}
		}
	...
	@RequestMapping(value = "/updateprogramstatus", 
			method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_JSON_VALUE, 
			produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<?> updateStatus(@RequestHeader(value = "Authorization") String authToken, @RequestBody Event event, @Context HttpServletRequest httpRequest){

		//save request details in PMP
		PMPAPIAccessLog pmpApiAccessLog = createPMPAPIAccessLog(null,httpRequest,StackTraceUtils.convertPojoToJson(event));

		//validate token details
		PMPResponse pmpResponse = authTokenVldtr.validateAuthToken(authToken, pmpApiAccessLog);
		if(pmpResponse instanceof ErrorResponse){
			return new ResponseEntity<PMPResponse>(pmpResponse, HttpStatus.OK);
		}

		LOGGER.info("Trying to update the status for the event : {} ", event.getAutoGeneratedEventId());

		User user = userProfileService.loadUserByEmail(pmpApiAccessLog.getUsername());

		if (null == user) {
			LOGGER.info(DashboardConstants.USER_UNAVAILABLE_IN_PMP);
			ErrorResponse eResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED,DashboardConstants.USER_UNAVAILABLE_IN_PMP);
			pmpApiAccessLog.setErrorMessage(DashboardConstants.USER_UNAVAILABLE_IN_PMP);
			updatePMPAPIAccessLog(pmpApiAccessLog,ErrorConstants.STATUS_FAILED,DashboardConstants.USER_UNAVAILABLE_IN_PMP, StackTraceUtils.convertPojoToJson(eResponse));
			return new ResponseEntity<ErrorResponse>(eResponse, HttpStatus.BAD_REQUEST);
		}
		...
		if(emailList.size() == 0){
			emailList.add(pmpApiAccessLog.getUsername());
		}
		...
		if (null == event.getAutoGeneratedEventId() || event.getAutoGeneratedEventId().isEmpty()) {
			...
			updatePMPAPIAccessLog(pmpApiAccessLog, ErrorConstants.STATUS_FAILED, DashboardConstants.INVALID_OR_EMPTY_EVENTID, StackTraceUtils.convertPojoToJson(eResponse));
			return new ResponseEntity<ErrorResponse>(eResponse,HttpStatus.PRECONDITION_REQUIRED);

		}else if(null == event.getProgramStatus() || event.getProgramStatus().isEmpty()){

			ErrorResponse eResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED,DashboardConstants.INVALID_OR_EMPTY_STATUS);
			updatePMPAPIAccessLog(pmpApiAccessLog, ErrorConstants.STATUS_FAILED, DashboardConstants.INVALID_OR_EMPTY_STATUS, StackTraceUtils.convertPojoToJson(eResponse));
			return new ResponseEntity<ErrorResponse>(eResponse,HttpStatus.PRECONDITION_REQUIRED);
			...
			try{
				programId = programService.getProgramIdByEventId(event.getAutoGeneratedEventId());
				if (programId > 0) {
					Program program = programService.getProgramByEmailAndRole(emailList, user.getRole(), event.getAutoGeneratedEventId(),authToken,pmpApiAccessLog);
					if(null != program && program.getIsReadOnly().equalsIgnoreCase(CoordinatorAccessControlConstants.IS_READ_ONLY_FALSE)){

						if(!program.getProgramStatus().equalsIgnoreCase(ProgramStatus.completed.name())){

							if(ProgramStatus.contains(event.getProgramStatus().toLowerCase())){
								programService.updateStatus(emailList,user.getRole(),event.getAutoGeneratedEventId(),event.getProgramStatus().toUpperCase(),program);
							}else{
								ErrorResponse eResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED,DashboardConstants.STATUS_ENUM_UPDATE);
								updatePMPAPIAccessLog(pmpApiAccessLog, ErrorConstants.STATUS_FAILED, DashboardConstants.STATUS_ENUM_UPDATE, StackTraceUtils.convertPojoToJson(eResponse));
								return new ResponseEntity<ErrorResponse>(eResponse,HttpStatus.PRECONDITION_REQUIRED);
							}
						}else{
							ErrorResponse eResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED,DashboardConstants.EVENT_STATUS_COMPLETED);
							updatePMPAPIAccessLog(pmpApiAccessLog, ErrorConstants.STATUS_FAILED, DashboardConstants.EVENT_STATUS_COMPLETED, StackTraceUtils.convertPojoToJson(eResponse));
							return new ResponseEntity<ErrorResponse>(eResponse,HttpStatus.PRECONDITION_REQUIRED);
						}

					}else{
						ErrorResponse eResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED,DashboardConstants.INVALID_USER);
						updatePMPAPIAccessLog(pmpApiAccessLog, ErrorConstants.STATUS_FAILED, DashboardConstants.INVALID_USER, StackTraceUtils.convertPojoToJson(eResponse));
						return new ResponseEntity<ErrorResponse>(eResponse,HttpStatus.PRECONDITION_REQUIRED);
					}

					SuccessResponse sresponse = new SuccessResponse(ErrorConstants.STATUS_SUCCESS, DashboardConstants.STATUS_SUCESS_UPDATE);
					updatePMPAPIAccessLog(pmpApiAccessLog,ErrorConstants.STATUS_SUCCESS,DashboardConstants.STATUS_SUCESS_UPDATE, StackTraceUtils.convertPojoToJson(sresponse));
					return new ResponseEntity<SuccessResponse>(sresponse, HttpStatus.OK);
				} else {
					ErrorResponse eResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED,DashboardConstants.INVALID_EVENTID);
					updatePMPAPIAccessLog(pmpApiAccessLog, ErrorConstants.STATUS_FAILED, DashboardConstants.INVALID_EVENTID, StackTraceUtils.convertPojoToJson(eResponse));
					return new ResponseEntity<ErrorResponse>(eResponse,HttpStatus.PRECONDITION_REQUIRED);
				}
			} catch (Exception e) {
				LOGGER.error("Exception while fetching event for event Id   : {},{}", event.getAutoGeneratedEventId(),e);
				ErrorResponse eResponse = new ErrorResponse(ErrorConstants.STATUS_FAILED,DashboardConstants.PROCESSING_FAILED);
				updatePMPAPIAccessLog(pmpApiAccessLog, ErrorConstants.STATUS_FAILED, StackTraceUtils.convertStackTracetoString(e), StackTraceUtils.convertPojoToJson(eResponse));
				return new ResponseEntity<ErrorResponse>(eResponse, HttpStatus.INTERNAL_SERVER_ERROR);
			}
		}
	}
