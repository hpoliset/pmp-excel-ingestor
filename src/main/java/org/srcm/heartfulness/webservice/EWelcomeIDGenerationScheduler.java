...
public class EWelcomeIDGenerationScheduler {
	...
	public void generateEWelcomeIDsForTheParticipants() {
		LOGGER.debug("START : CRON : EWELCOMEID GENERATION : Scheduler to generate EwelcomeID's for the participants started at - " + new Date());
		...
		for (Participant participant : participants) {
			try {
				...
				if ("success".equalsIgnoreCase(eWelcomeID)) {
					LOGGER.debug(
							"CRON : EWELCOMEID GENERATION : eWelcomeID generated successfully to the participant : {},EventID: {},EwelcomeID: {} ",
							participant.getPrintName(),participant.getProgram().getAutoGeneratedEventId(),eWelcomeID);
					...
					accessLog.setResponseBody("Email:"+participant.getEmail()+" ,EventID:"+participant.getProgram().getAutoGeneratedEventId()+" ,SeqId:"+participant.getSeqId()+" ,EwelcomeID:"+participant.getWelcomeCardNumber()+" ,Issued Date:"+participant.getWelcomeCardDate()+" ,EwelcomeID State:"+participant.getEwelcomeIdState()+" ,EwelcomeID informed:"+participant.getIsEwelcomeIdInformed());
					accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
					apiAccessLogService.updatePmpAPIAccessLog(accessLog);
					LOGGER.debug("CRON : EWELCOMEID GENERATION :Updated log successfully");
				}else{
					LOGGER.debug(
							"CRON : EWELCOMEID GENERATION : eWelcomeID generation failed to the participant : {} ,EventID: {},EwelcomeID remarks: {} ",
							participant.getPrintName(),participant.getProgram().getAutoGeneratedEventId(),eWelcomeID);
					...
					accessLog.setResponseBody("Email:"+participant.getEmail()+" ,EventID:"+participant.getProgram().getAutoGeneratedEventId()+" ,SeqId:"+participant.getSeqId()+" ,EwelcomeID State:"+participant.getEwelcomeIdState()+" ,EwelcomeID informed:"+participant.getIsEwelcomeIdInformed());
					accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
					apiAccessLogService.updatePmpAPIAccessLog(accessLog);
				...
				LOGGER.debug(
						"CRON : EWELCOMEID GENERATION : Error while generating EWelcomeID for the participant :"
								+ " SeqID : {} , Name : {} , Email : {} ", participant.getSeqId(),participant.getPrintName(), participant.getEmail());
		...
		LOGGER.debug("END : CRON : EWELCOMEID GENERATION : Scheduler to generate EwelcomeID's for the participants completed at - " + new Date());
	}
