...
public class EWelcomeIDGenerationScheduler {
	...
	public void generateEWelcomeIDsForTheParticipants() {
		...
		try {
			...
			LOGGER.debug("Error While fetching participants from DB : Exception : {} ",
		...
		for (Participant participant : participants) {
			...
			try {
				...
				LOGGER.debug("Error while inserting data into pmp api access log table. Exception: {} ",
						StackTraceUtils.convertStackTracetoString(e));
			}
			try {
				 ...
				 * + " SeqID : {} , Name : {} , Email : {} ",
				 * participant.getSeqId(), participant.getPrintName(),
				 * participant.getEmail()); LOGGER.debug(
				 * "Scheduler to generate EwelcomeID's : Exception :  {} ",
				 * StackTraceUtils.convertStackTracetoString(e));
				 * participant.setEwelcomeIdRemarks(e.getMessage());
				 * participant.
				 * setEwelcomeIdState(PMPConstants.EWELCOMEID_FAILED_STATE);
				 * participant.setIsEwelcomeIdInformed(0);
				 * participantRepository.save(participant); LOGGER.debug(
				 * "CRON : EWELCOMEID GENERATION : Failed case: Participant details persisted successfully"
				 * ); } catch (Exception ex) { LOGGER.debug(
				 * "Exception while persisting participant details : {} ",
