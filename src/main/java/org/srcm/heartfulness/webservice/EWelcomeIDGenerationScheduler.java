...
public class EWelcomeIDGenerationScheduler {
	...
	public void generateEWelcomeIDsForParticipants() {
		...
		LOGGER.info("CRON : EWELCOMEID GENERATION : Total no. of Events to generate eWelcomeID : {} ",programIds.size());
		...
		if (!programIds.isEmpty() && programIds.size() > 0) {
			...
			for (Integer programId : programIds) {
				...
				Program program = programService.getProgramDetailsToGenerateEwelcomeIDById(programId);
				LOGGER.info("CRON : EWELCOMEID GENERATION : EventID: {} - Total no. of participants to generate eWelcomeID : {} ",program.getAutoGeneratedEventId(), participants.size());

				for (Participant participant : participants) {
					participant.setProgram(program);
					PMPAPIAccessLog accessLog = createPMPAPIAccessLog(null,null,StackTraceUtils.convertPojoToJson(participant));
					LOGGER.info("Participant Name : "+participant.getPrintName()+",Participant email : "+participant.getEmail());
					LOGGER.info("Event ID : "+participant.getProgram().getAutoGeneratedEventId() + ",Event ID : "+participant.getProgram().getProgramId());
					try {
						...
						if(participant.getSessionId() > 0){
							try{
								int firstSittingBy  = sessionDtlsRepo.getFirstSittingBySessionId(participant.getSessionId());
								participant.setFirstSittingBy(firstSittingBy);
							} catch(Exception ex){
								LOGGER.error("Issue while fetching first sitting by for session Id {} and event Id {}",participant.getSessionId(),program.getAutoGeneratedEventId());
								if (0 == program.getFirstSittingBy()) {
						...
						if (null == isvalid) {
							...
							if ("success".equalsIgnoreCase(eWelcomeID)) {
								...
								updatePMPAPIAccessLog(accessLog, ErrorConstants.STATUS_FAILED, eWelcomeID, responseBody);
							}
						} else {
							try {
								for (Participant participant2 : participants) {
							...
							String responseBody = "Email:" + participant.getEmail() + " ,EventID:" 
									...
									+ participant.getSeqId() + " ,EwelcomeID State:" + participant.getEwelcomeIdState() 
									+ " ,EwelcomeID informed:" + participant.getIsEwelcomeIdInformed();
							updatePMPAPIAccessLog(accessLog, ErrorConstants.STATUS_FAILED, isvalid, responseBody);
						}
					} catch (Exception e) {
						LOGGER.error("Scheduler to generate EwelcomeID's : Exception :  {} ",StackTraceUtils.convertStackTracetoString(e));
						String responseBody = "Email:" + participant.getEmail() + " ,EventID:" + participant.getProgram().getAutoGeneratedEventId() + " ,SeqId:"
								+ participant.getSeqId() + " ,EwelcomeID State:" + participant.getEwelcomeIdState() + " ,EwelcomeID informed:" + participant.getIsEwelcomeIdInformed();

						updatePMPAPIAccessLog(accessLog, ErrorConstants.STATUS_FAILED, StackTraceUtils.convertPojoToJson(e), responseBody);
					}

				}
			}
			LOGGER.info("END : CRON : EWELCOMEID GENERATION : Scheduler to generate EwelcomeID's for the participants completed at - "+ new Date());
		} else {
			LOGGER.info("END : CRON : EWELCOMEID GENERATION : No participants available to generate EwelcomeID's on - "+ new Date());
		}
	}
