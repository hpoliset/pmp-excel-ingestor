...
public class EventController {
	...
	public String showEventForm(@RequestParam(required = false, value = "id") String encryptedValue,
		...
		/*
		 * String response = "redirect:/login"; try { if
		 * (request.getSession().getAttribute("Authentication") != null) {
		 * authHelper.setcurrentUsertoContext(request.getSession()); String
		 * eventId = encryptedValue; try { eventId =
		 * aesEncryptDecrypt.decrypt(encryptedValue,
		 * env.getProperty("security.encrypt.token")); } catch (Exception ex) {
		 * LOGGER.debug("Invalid Event ID :" + eventId +
		 * " Please provide a valid Event ID."); } Program program =
		 * programService.findByAutoGeneratedEventId(eventId); if
		 * (program.getProgramId() > 0) { model.addAttribute("program",
		 * program); response = "programform_new"; } else {
		 * model.addAttribute("message",
		 * "Incorrect Event ID. Please provide a valid Event ID."); response =
		 * "eventsuccess"; } } else {
		 * redirectAttributes.addFlashAttribute("redirecturl",
		 * "/updateevent?id=" + encryptedValue); response = "redirect:/login"; }
		 * } catch (Exception e) { return response; } return response;
		 */
