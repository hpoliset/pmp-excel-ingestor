	...
	public String createParticipant(SMS sms) {
		...
		if (contents.length > 0 && contents.length <= 6) {
			...
			if (eventId != null && !eventId.isEmpty() && eventId.length() == SMSConstants.SMS_EVENT_ID_LENGTH) {
				...
				if (eventId.length() == 8 && contents.length == 3 && contents[2].length() == 4) {
					seqNum = contents[2];
					Participant participant = participantRepository.getParticipantByIntroIdAndMobileNo(eventId, seqNum);
					...
					response = SMSConstants.SMS_EWELCOME_RESPONSE_INVALID_SEQ_FORMAT_1 + seqNum
							+ SMSConstants.SMS_EWELCOME_RESPONSE_INVALID_SEQ_FORMAT_2;
				}
			} else if (eventId != null && !eventId.isEmpty() && eventId.equalsIgnoreCase(SMSConstants.SMS_HELP_KEYWORD)) {
