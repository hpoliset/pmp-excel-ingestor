...
public class ProgramServiceImpl implements ProgramService {
	 ...
	 */
	/*
	 * @Override public List<Event> getEventListByEmail(String email, boolean
	 * isAdmin) { List<Event> eventList = new ArrayList<Event>(); List<Program>
	 * programList = programRepository.getEventsByEmail(email, isAdmin);
	 * SimpleDateFormat convertedsdf = new SimpleDateFormat("dd-MM-yyyy"); for
	 * (Program program : programList) { Event event = new Event();
	 * event.setAutoGeneratedEventId(program.getAutoGeneratedEventId());
	 * event.setProgramChannel(program.getProgramChannel());
	 * event.setProgramName(program.getProgramName()); if (null ==
	 * program.getProgramStartDate()) { event.setProgramStartDate(""); } else {
	 * try {
	 * event.setProgramStartDate(convertedsdf.format(program.getProgramStartDate
	 * ())); } catch (Exception e) { event.setProgramStartDate(""); } } if (null
	 * == program.getProgramEndDate()) { event.setProgramEndDate(""); } else {
	 * try {
	 * event.setProgramEndDate(convertedsdf.format(program.getProgramEndDate
	 * ())); } catch (Exception e) { event.setProgramEndDate(""); } }
	 ...
	 * event.setRemarks(program.getRemarks()); eventList.add(event); }
	 * 
	 * return eventList; }
	 */
	@Override
		...
		for (Event event : events) {
			...
			if (!errors.isEmpty()) {
				...
				if (null != event.getPreceptorIdCardNumber() && !event.getPreceptorIdCardNumber().isEmpty()) {
					...
					program.setPreceptorIdCardNumber(event.getPreceptorIdCardNumber());
					preceptorIdValidationResponse = validatePreceptorIDcardNumber(program, id, errors);
				...
				if (errors.isEmpty()) {
					...
					if(!persistedPgrm.getCoordinatorEmail().equalsIgnoreCase(existingCoordinatorEmailId)){
						...
						String isCoordinatorEmailIdvalid = coordinatorAccessControlService.validateCoordinatorEmailID(program, id);
						...
						if(null == isCoordinatorEmailIdvalid){
							System.out.println("1. Need to send mail to coordinator with link to access to dashboard..");
							coordinatorAccessControlMail.sendMailToCoordinatorWithLinktoAccessDashboard(accessControlEmail);
						}else{
							System.out.println("2. Need to send mail to coordinator to create profile with MYSRCM..");
					...
					if (preceptorIDModified & null != preceptorIdValidationResponse) {
						System.out.println("PreceptorID changed..");
						participantService.updatePartcipantEWelcomeIDStatuswithParticipantID(
								persistedPgrm.getProgramId(), PMPConstants.EWELCOMEID_FAILED_STATE,
								preceptorIdValidationResponse);
					} else if (preceptorIDModified & null == preceptorIdValidationResponse) {
						participantService.updatePartcipantEWelcomeIDStatuswithParticipantID(
								persistedPgrm.getProgramId(), PMPConstants.EWELCOMEID_TO_BE_CREATED_STATE, null);
					}
	...
	private String validatePreceptorIDcardNumber(Program program, int id, Map<String, String> errors)
			throws HttpClientErrorException, JsonParseException, JsonMappingException, IOException {
		if(errors.isEmpty()){
			return coordinatorAccessControlService.validatePreceptorIDCardNumberandCreateUser(program, id, PMPConstants.CREATED_SOURCE_DASHBOARD);
		}
		return null;
	}
	 ...
	 */
	/*
	 * @Override public List<Event> searchEvents(SearchRequest searchRequest) {
	 * List<Event> eventList = new ArrayList<Event>(); for (EventSearchField
	 * searchField : EventSearchField.values()) { if
	 * (searchField.name().equals(searchRequest.getSearchField())) {
	 * searchRequest.setSearchField(searchField.getValue()); } if
	 * (searchField.name().equals(searchRequest.getSortBy())) {
	 * searchRequest.setSortBy(searchField.getValue()); } }
	 * 
	 * List<Program> programList =
	 * programRepository.searchEvents(searchRequest); SimpleDateFormat
	 * convertedsdf = new SimpleDateFormat(PMPConstants.DATE_FORMAT); for
	 * (Program program : programList) { Event event = new Event();
	 * event.setAutoGeneratedEventId(program.getAutoGeneratedEventId());
	 * event.setProgramChannel(program.getProgramChannel());
	 * event.setProgramName(program.getProgramName()); if (null ==
			...
			try {
				...
				try {
					fetchEwelcomeIDAPIAccessLogDetails = new PMPAPIAccessLogDetails(id,
							EndpointConstants.ABHYASI_INFO_URI + "?email_exact=" + participant.getEmail(),
				...
				try {
					...
					fetchEwelcomeIDAPIAccessLogDetails
							.setResponseBody(StackTraceUtils.convertPojoToJson(abhyasiResult));
	...
	public int getProgramCountWithUserRoleAndEmailId(String email, String role) {
		return programRepository.getProgramCountWithUserRoleAndEmailId(email, role);
	}

	@Override
	public List<Event> getEventListByEmailAndRole(String email, String role, int offset, int pageSize) {
		List<Event> eventList = new ArrayList<Event>();
		List<Program> programList = programRepository.getEventsByEmailAndRole(email, role, offset, pageSize);
		SimpleDateFormat convertedsdf = new SimpleDateFormat("dd-MM-yyyy");
		for (Program program : programList) {
			Event event = new Event();
			event.setAutoGeneratedEventId(program.getAutoGeneratedEventId());
	...
	public int getProgramCountOfOthersEventByEmailIdAndRole(String email, String role) {
		return programRepository.getProgramCountOfOthersEventByEmailIdAndRole(email, role);
	}

	@Override
	public List<Event> getOthersEventListByEmailIdAndRole(String email, String role, int offset, int pageSize) {
		List<Event> eventList = new ArrayList<Event>();
		List<Program> programList = programRepository.getOthersEventListByEmailIdAndRole(email, role, offset, pageSize);
		SimpleDateFormat convertedsdf = new SimpleDateFormat("dd-MM-yyyy");
		for (Program program : programList) {
			Event event = new Event();
			event.setAutoGeneratedEventId(program.getAutoGeneratedEventId());
	...
	public List<Event> searchEventsWithUserRoleAndEmailId(SearchRequest searchRequest, String email, String role,
		...
		List<Program> programList = programRepository.searchEventsWithUserRoleAndEmailId(searchRequest,email,role,offset);
		SimpleDateFormat convertedsdf = new SimpleDateFormat(PMPConstants.DATE_FORMAT);
		for (Program program : programList) {
			Event event = new Event();
			event.setAutoGeneratedEventId(program.getAutoGeneratedEventId());
