...
public class ProgramServiceImpl implements ProgramService {
		...
		for (Event event : events) {
			...
			if (!errors.isEmpty()) {
				...
				if (null != event.getAutoGeneratedEventId() && !event.getAutoGeneratedEventId().isEmpty()) {
					// int pgmID =
					// getProgramIdByEventId(event.getAutoGeneratedEventId());
				...
				if (errors.isEmpty()) {
					...
					if (preceptorIDModified & null != preceptorIdValidationResponse) {
						participantService.updatePartcipantEWelcomeIDStatuswithProgramID(
	...
	public void createOrUpdateProgram(Event event, int id) throws HttpClientErrorException, JsonParseException,
		...
		if (!errors.isEmpty()) {
			...
			program.setRemarks(event.getRemarks());
			if(null == event.getIsEventDisabled() || event.getIsEventDisabled().trim().isEmpty()){
				errors.put("Is Event Disabled", "Event disabled state is required");
				event.setErrors(errors);
			}else if(event.getIsEventDisabled().trim().equals(EventDetailsUploadConstants.EWELCOME_ID_DISABLED_STATE)){
				program.setIsEventDisabled(EventDetailsUploadConstants.EWELCOME_ID_DISABLED_STATE);
				event.setIsEventDisabled(EventDetailsUploadConstants.EWELCOME_ID_DISABLED_STATE);
			...
			if (errors.isEmpty()) {
				Program persistedPgrm = programRepository.saveProgram(program);
				if (preceptorIDModified & null != preceptorIdValidationResponse) {
					participantService.updatePartcipantEWelcomeIDStatuswithProgramID(persistedPgrm.getProgramId(),
