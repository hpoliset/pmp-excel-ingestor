	...
	private String fetchParticipantEwelcomeID(Participant participant, int id) {
		...
		if(null != participant.getEmail() && !participant.getEmail().isEmpty()){
			try {
				...
				LOGGER.error("HTTP CLIENT ERROR : Error While fetching already generated ewelcomeId of the participant. NAME: {}, EMAIL: {}, EXCEPTION: {} ",participant.getPrintName(),participant.getEmail(),e.getMessage());
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
				...
				LOGGER.error("JSONPARSE/JSONMAPPING ERROR : Error While fetching already generated ewelcomeId of the participant. NAME: {}, EMAIL: {}, EXCEPTION: {} ",participant.getPrintName(),participant.getEmail(),e.getMessage());
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
					fetchEwelcomeIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_FAILED);
					fetchEwelcomeIDAPIAccessLogDetails.setErrorMessage(StackTraceUtils.convertStackTracetoString(e));
				...
				LOGGER.error("Exception : Error While fetching already generated ewelcomeId of the participant. NAME: {}, EMAIL: {}, EXCEPTION: {} ",participant.getPrintName(),participant.getEmail(),e.getMessage());
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
					fetchEwelcomeIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_FAILED);
					fetchEwelcomeIDAPIAccessLogDetails.setErrorMessage(StackTraceUtils.convertStackTracetoString(e));
