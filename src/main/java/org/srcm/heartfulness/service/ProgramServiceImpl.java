...
/*	@Override
	...
	public String generateeWelcomeID(Participant participant, int id) {
		LOGGER.info(
				"CRON : EWELCOMEID GENERATION : loggerID:{} : eWelcomeID generation to the participant : {}, SeqID:{}, EventID: {}, ",id,
				participant.getPrintName(), participant.getSeqId(), participant.getProgram()
						.getAutoGeneratedEventId());
								...
								if (citiesObj instanceof CitiesAPIResponse) {
									...
									if (null != citiesAPIResponse) {
										...
										if (aspirantobj instanceof String) {
											...
											if (null != eWelcomeID) {
												LOGGER.debug(
														"CRON : EWELCOMEID GENERATION : loggerID:{} : eWelcomeID generated successfully to the participant : {}, SeqID:{}, EventID: {}, EwelcomeID: {} ",id,
														participant.getPrintName(), participant.getSeqId(), participant.getProgram()
	...
	private String fetchParticipantEwelcomeID(Participant participant, int id) {
		...
		if(null != participant.getEmail() && !participant.getEmail().isEmpty()){
			try {
				...
				LOGGER.error("HTTP CLIENT ERROR : Error While fetching already generated ewelcomeId of the participant. NAME: {}, EMAIL: {}, EXCEPTION: {} ",participant.getPrintName(),participant.getEmail(),StackTraceUtils.convertStackTracetoString(e));
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
				...
				LOGGER.error("JSONPARSE/JSONMAPPING ERROR : Error While fetching already generated ewelcomeId of the participant. NAME: {}, EMAIL: {}, EXCEPTION: {} ",participant.getPrintName(),participant.getEmail(),StackTraceUtils.convertStackTracetoString(e));
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
					fetchEwelcomeIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_FAILED);
					fetchEwelcomeIDAPIAccessLogDetails.setErrorMessage(StackTraceUtils.convertStackTracetoString(e));
				...
				LOGGER.error("Exception : Error While fetching already generated ewelcomeId of the participant. NAME: {}, EMAIL: {}, EXCEPTION: {} ",participant.getPrintName(),participant.getEmail(),StackTraceUtils.convertStackTracetoString(e));
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
					fetchEwelcomeIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_FAILED);
					fetchEwelcomeIDAPIAccessLogDetails.setErrorMessage(StackTraceUtils.convertStackTracetoString(e));
	...
	public String validatePreceptorIDCardNumber(Program program, int id) {
		LOGGER.info("Validating preceptor ID :{} ,  loggerID:{} ",program.getPreceptorIdCardNumber(),id);
		PMPAPIAccessLogDetails accessLogDetails = null;
		if (null == program.getPrefectId() || program.getPrefectId().isEmpty()) {
