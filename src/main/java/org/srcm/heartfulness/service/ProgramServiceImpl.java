	...
	public List<Event> createOrUpdateEvent(List<Event> events, int id) throws HttpClientErrorException,
		...
		for (Event event : events) {
			...
			if (!errors.isEmpty()) {
				...
				// int pgmID=getProgramIdByEventId(event.getAutoGeneratedEventId());
				// Program pgm = getProgramById(pgmID);
		...
		if (result.getUserProfile().length > 0) {
			...
			if (null != userProfile) {
				if (true == userProfile.isIs_prefect() && 0 != userProfile.getId()) {
					...
					program.setPreceptorIdCardNumber(program.getPreceptorIdCardNumber());
					program.setPreceptorName(userProfile.getName());
					return null;
				} else {
					return "Specified PreceptorId Card Number is not authorized.";
				}
			} else {
