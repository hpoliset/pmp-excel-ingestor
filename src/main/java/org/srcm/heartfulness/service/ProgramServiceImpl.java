...
public class ProgramServiceImpl implements ProgramService {
	...
	// LoggerFactory.getLogger(ProgramServiceImpl.class);
	...
	public List<Event> createOrUpdateEvent(List<Event> events, int id) throws HttpClientErrorException,
		...
		for (Event event : events) {
			...
			if (!errors.isEmpty()) {
				...
				Program program = programRepository.findByAutoGeneratedEventId(null != event.getAutoGeneratedEventId() ? event.getAutoGeneratedEventId() : null); //new Program();
				...
				if(null != event.getAutoGeneratedEventId() && !event.getAutoGeneratedEventId().isEmpty()){
					...
					if(!pgm.getPreceptorIdCardNumber().equalsIgnoreCase(event.getPreceptorIdCardNumber())){
						String preceptorIdValidationResponse= validatePreceptorIDcardNumber(program,event);
						if(null != preceptorIdValidationResponse){
							errors.put("PreceptorId Card Number",preceptorIdValidationResponse);
							event.setErrors(errors);
							participantService.updatePartcipantEWelcomeIDStatuswithParticipantID(pgmID,PMPConstants.EWELCOMEID_FAILED_STATE,preceptorIdValidationResponse);
						}else{
							event.setPreceptorName(program.getPreceptorName());
							/*program.setPrefectId(program.getPrefectId());
							program.setAbyasiRefNo(program.getAbyasiRefNo());
							program.setPreceptorName(program.getPreceptorName());
							program.setPreceptorIdCardNumber(pgm.getPreceptorIdCardNumber());*/
							participantService.updatePartcipantEWelcomeIDStatuswithParticipantID(pgmID,PMPConstants.EWELCOMEID_TO_BE_CREATED_STATE,null);
						}
					}
				}else{
					String preceptorIdValidationResponse= validatePreceptorIDcardNumber(program,event);
					...
					if (result.getUserProfile().length > 0) {
						...
						if (null != userProfile) {
							if (true == userProfile.isIs_prefect() && 0 != userProfile.getPrefect_id()) {
								...
								event.setPreceptorName(userProfile.getName());
							} else {
	...
	private String validatePreceptorIDcardNumber(Program program, Event event) throws HttpClientErrorException, JsonParseException, JsonMappingException, IOException {
