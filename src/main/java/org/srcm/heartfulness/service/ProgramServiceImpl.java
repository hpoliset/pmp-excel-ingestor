...
public class ProgramServiceImpl implements ProgramService {
			...
			if (!errors.isEmpty()) {
				...
				if (errors.isEmpty()) {
					...
					if (!persistedPgrm.getCoordinatorEmail().equalsIgnoreCase(existingCoordinatorEmailId)) {
						...
						String isCoordinatorEmailIdvalid = coordinatorAccessControlService.validateCoordinatorEmailID(
								program, id);
						...
						if (null == isCoordinatorEmailIdvalid) {
							System.out
									.println("1. Need to send mail to coordinator with link to access to dashboard..");
							coordinatorAccessControlMail
									.sendMailToCoordinatorWithLinktoAccessDashboard(accessControlEmail);
						} else {
							System.out.println("2. Need to send mail to coordinator to create profile with MYSRCM..");
							coordinatorAccessControlMail
									.sendMailToCoordinatorWithLinktoCreateProfile(accessControlEmail);
						}
	...
	@Override
	public List<Event> searchEventsWithUserRoleAndEmailId(SearchRequest searchRequest, List<String> emailList/*String email*/, String role, int offset) {
		List<Event> eventList = new ArrayList<Event>();
		/*
		 * for (EventSearchField searchField : EventSearchField.values()) { if
				...
				if (!persistedPgrm.getCoordinatorEmail().equalsIgnoreCase(existingCoordinatorEmailId)) {
					...
					if (null == isCoordinatorEmailIdvalid) {
						...
						Runnable task = new Runnable() {
							...
							public void run() {
								System.out
										.println("2. Need to send mail to coordinator to create profile with MYSRCM.");
								coordinatorAccessControlMail
										.sendMailToCoordinatorWithLinktoCreateProfile(accessControlEmail);
							}
						};
