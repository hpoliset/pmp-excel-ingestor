...
public class AmazonS3ServiceImpl implements AmazonS3Service {
	...
	@Autowired
	ExtensionFilter extentionFilter;
	...
	@Override
	public ResponseEntity<List<Response>> uploadObjectInAWSAndUpdateEvent(String eventId, 
			String prmsGvnBy, String prmsGvrDesignation, 
			String prmsGvrPhone, String prmsGvrEmailId,
			MultipartFile[] multipartFiles, PMPAPIAccessLog accessLog) {
		Program program = programRepository.findByAutoGeneratedEventId(eventId);
		List<Response> listOfResponse = new ArrayList<Response>();
		for (MultipartFile multipartFile : multipartFiles) {
			...
			try {
				...
				ProgramPermissionLetterdetails programPermissionLetterdetails = new ProgramPermissionLetterdetails(
						program.getProgramId(), multipartFile.getOriginalFilename(), permissionLetterPath,
						accessLog.getUsername(), prmsGvnBy, prmsGvrDesignation, prmsGvrPhone, prmsGvrEmailId);
				programRepository.saveProgramPermissionLetterDetails(programPermissionLetterdetails);
	...
	@Override
	public ResponseEntity<List<Response>> uploadTestimonialInAWSAndUpdateEvent(String eventId,
			MultipartFile[] multipartFiles, PMPAPIAccessLog accessLog) {
		Program program = programRepository.findByAutoGeneratedEventId(eventId);
		List<Response> listOfResponse = new ArrayList<Response>();
		for (MultipartFile multipartFile : multipartFiles) {
			Response response = null;
			try {
				String testimonialPath = program.getAutoGeneratedEventId() + ExpressionConstants.PATH_SEPARATER + DashboardConstants.TESTIMONIAL_FOLDER
						+ ExpressionConstants.PATH_SEPARATER + multipartFile.getOriginalFilename();

				calculateSignatureAndUploadObjectToAWS(multipartFile, testimonialPath);

				ProgramTestimonialDetails testimonialDetails = new ProgramTestimonialDetails(program.getProgramId(),
						multipartFile.getOriginalFilename(), testimonialPath, extentionFilter.getFileType(multipartFile
								.getOriginalFilename()), accessLog.getUsername());

				programRepository.saveProgramTestimonialDetails(testimonialDetails);
	 ...
	 * 
	 * @see org.srcm.heartfulness.service.AmazonS3Service#
	 * createPresignedURLForTestimonials(java .lang.String, java.lang.String,
	 * org.srcm.heartfulness.model.PMPAPIAccessLog)
	 */
	@Override
	public ResponseEntity<Map<String, Object>> createPresignedURLForTestimonials(String eventId,
			PMPAPIAccessLog accessLog) {
		Map<String, Object> response = new HashMap<String, Object>();
		Program program = programRepository.findByAutoGeneratedEventId(eventId);
		List<ProgramTestimonialDetails> listOfTestimonials = programRepository.getListOfTestimonials(program.getProgramId());
		for (ProgramTestimonialDetails testimonial : listOfTestimonials) {
			try {
				String presignedURL = amazonS3Interface.generatePresignedUrl(program.getAutoGeneratedEventId()
						+ ExpressionConstants.PATH_SEPARATER + DashboardConstants.TESTIMONIAL_FOLDER + ExpressionConstants.PATH_SEPARATER
						+ testimonial.getTestimonialName());
				testimonial.setPresignedURL(presignedURL);
				response.put(testimonial.getTestimonialName(), testimonial);
				accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
				accessLog
						.setResponseBody(StackTraceUtils.convertPojoToJson(StackTraceUtils.convertPojoToJson(response)));
				apiAccessLogService.updatePmpAPIAccessLog(accessLog);
				...
				accessLog.setResponseBody(StackTraceUtils.convertPojoToJson(response));
				apiAccessLogService.updatePmpAPIAccessLog(accessLog);
				response.put(testimonial.getTestimonialName(),
						"Failed to fetch image path for  " + testimonial.getTestimonialName());
			} catch (AmazonClientException ace) {
				LOGGER.error("Caught an AmazonClientException, which " + "means the client encountered "
						+ "an internal error while trying to " + "communicate with S3, "
						+ "such as not being able to access the network.Exception : {}", ace);
	 ...
	 * 
	 * @see org.srcm.heartfulness.service.AmazonS3Service#
	 * uploadListOfObjectsInAWSForSessionV2(java.lang.String, java.lang.String,
	 * org.springframework.web.multipart.MultipartFile[],
	 * org.srcm.heartfulness.model.PMPAPIAccessLog)
	 */
	@Override
	public ResponseEntity<List<Response>> uploadListOfFilesInAWSForSession(String eventId, String sessionId,
			MultipartFile[] multipartFiles, PMPAPIAccessLog accessLog) {
		Program program = programRepository.findByAutoGeneratedEventId(eventId);
		int sessionDetailsId = sessionDetailsService.getSessionDetailsIdBySessionIdandProgramId(sessionId,
				program.getProgramId());
		...
		for (MultipartFile multipartFile : multipartFiles) {
			...
			try {
				...
				SessionImageDetails sessionFiles = new SessionImageDetails(sessionDetailsId,
						multipartFile.getOriginalFilename(), sessionFilesPath, extentionFilter.getSessionFileType(multipartFile.getOriginalFilename()), accessLog.getUsername());
				sessionDetailsService.saveSessionFilesWithType(sessionFiles);
				response = new Response(ErrorConstants.STATUS_SUCCESS, multipartFile.getOriginalFilename()
						+ " uploaded successfully");
				accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
				accessLog
	 ...
	 * 
	 * @see org.srcm.heartfulness.service.AmazonS3Service#
	 * createPresignedURLForSessionImagesV2(java.lang.String, java.lang.String,
	 * org.srcm.heartfulness.model.PMPAPIAccessLog)
	 */
	@Override
	public ResponseEntity<?> createPresignedURLForSessionFiles(String eventId, String sessionId,PMPAPIAccessLog accessLog) {
		...
		for (SessionImageDetails sessionImage : listOfImages) {
			try {
				String presignedURL = amazonS3Interface.generatePresignedUrl(sessionImage.getImagePath());
				sessionImage.setPresignedURL(presignedURL);
				response.put(sessionImage.getImageName(), sessionImage);
				accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
				accessLog
						.setResponseBody(StackTraceUtils.convertPojoToJson(StackTraceUtils.convertPojoToJson(response)));
				apiAccessLogService.updatePmpAPIAccessLog(accessLog);
	...
	@Override
	public ResponseEntity<Map<String, Object>> createPresignedURLWithDetails(String eventId,PMPAPIAccessLog accessLog) {
		Map<String, Object> response = new HashMap<String, Object>();
		Program program = programRepository.findByAutoGeneratedEventId(eventId);
		List<ProgramPermissionLetterdetails> listOfPermissionLetters = programRepository
				.getListOfPermissionLetters(program.getProgramId());
		for (ProgramPermissionLetterdetails programPermissionLetter : listOfPermissionLetters) {
			try {
				String presignedURL = amazonS3Interface.generatePresignedUrl(program.getAutoGeneratedEventId()+ExpressionConstants.PATH_SEPARATER+programPermissionLetter.getPermissionLetterName());
				programPermissionLetter.setPresignedURL(presignedURL);
				response.put(programPermissionLetter.getPermissionLetterName(), programPermissionLetter);
				accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
				accessLog
						.setResponseBody(StackTraceUtils.convertPojoToJson(StackTraceUtils.convertPojoToJson(response)));
				apiAccessLogService.updatePmpAPIAccessLog(accessLog);
