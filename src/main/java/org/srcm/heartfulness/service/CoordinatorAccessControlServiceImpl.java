...
public class CoordinatorAccessControlServiceImpl implements CoordinatorAccessControlService {
	...
	public CoordinatorAccessControlResponse addSecondaryCoordinatorRequest(String autoGeneratedEventId,
		...
		ProgramCoordinators pgrmCoodDetails = new ProgramCoordinators();
		try {
		...
		if (null != pgrmCoodDetails.getEmail()) {
		...
		if (reqAlreadyRaised == 0) {
			...
			try {
				 ...
				 * PMPAPIAccessLogDetails(accessLog.getId(),
				 * EndpointConstants.ABHYASI_INFO_URI,
				 * DateUtils.getCurrentTimeInMilliSec(), null,
				 * ErrorConstants.STATUS_FAILED, null,
				 * StackTraceUtils.convertPojoToJson("Request : " +
				 * program.getPreceptorIdCardNumber() + " ,Event ID : " +
				 * autoGeneratedEventId)); try{
				 * apiAccessLogService.createPmpAPIAccesslogDetails
				 * (accessLogDetails); }catch(Exception ex){
				 * LOGGER.error("failed to create api access log details",ex); }
				 */
				/*
				 * String preceptorName = "",preceptorEmail = ""; try{
				 * AbhyasiResult abhyasiProfile =
				 * srcmRestTemplate.getAbyasiProfile
				 * (program.getPreceptorIdCardNumber()); preceptorName =
				 * abhyasiProfile.getUserProfile()[0].getFirst_name();
				 * preceptorEmail =
				 * abhyasiProfile.getUserProfile()[0].getEmail();
				 * }catch(Exception ex){
				 * LOGGER.error("Failed to retrieve preceptor email from MYSRCM"
				 * ,ex); }
				 */
				/*
				 * try{
				 ...
				 * accessLogDetails
				 * .setResponseBody("Details received from MYSRCM, preceptor name "
				 * +preceptorName +" and preceptor email "+preceptorEmail);
				 * accessLogDetails
				...
				try {
					coordinatorAccessControlMail.sendRequestMailToCoordinatorAndPreceptor(program.getCoordinatorName(),
							program.getCoordinatorEmail(), autoGeneratedEventId);
					PMPMailLog pmpMailLog = new PMPMailLog(String.valueOf(program.getProgramId()),
							null == program.getCoordinatorEmail() ? "" : program.getCoordinatorEmail(),
					...
					PMPMailLog pmpMailLog = new PMPMailLog(String.valueOf(program.getProgramId()),
							null == program.getCoordinatorEmail() ? "" : program.getCoordinatorEmail(),
							EmailLogConstants.REQUEST_MAIL_TO_COORDINATOR, EmailLogConstants.STATUS_FAILED,
				...
				try {
					if (null != pgrmCoodDetails.getEmail()) {
						coordinatorAccessControlMail.sendRequestMailToCoordinatorAndPreceptor(
								null == pgrmCoodDetails.getName() ? "" : pgrmCoodDetails.getName(),
								pgrmCoodDetails.getEmail(), autoGeneratedEventId);
					}
					PMPMailLog pmpMailLog = new PMPMailLog(String.valueOf(program.getProgramId()),
							null == pgrmCoodDetails.getEmail() ? "" : pgrmCoodDetails.getEmail(),
					...
					LOGGER.error("Failed to send request mail to preceptor for event Id " + autoGeneratedEventId);
					PMPMailLog pmpMailLog = new PMPMailLog(String.valueOf(program.getProgramId()),
							null == pgrmCoodDetails.getEmail() ? "" : pgrmCoodDetails.getEmail(),
							EmailLogConstants.REQUEST_MAIL_TO_PRECEPTOR, EmailLogConstants.STATUS_FAILED,
							StackTraceUtils.convertStackTracetoString(ex));
					try {
						mailLogRepository.createMailLog(pmpMailLog);
					} catch (Exception e) {
	...
	public CoordinatorAccessControlResponse approveSecondaryCoordinatorRequest(String approvedBy,
		...
		try {
			approvedStatus = coordntrAccssCntrlRepo.approveSecondaryCoordinatorRequest(program.getProgramId(),
					approvedBy, pgrmCoordinators.getEmail());
		} catch (DataAccessException daex) {
		...
		if (approvedStatus > 0) {
			User user = coordntrAccssCntrlRepo.getUserbyUserEmail(pgrmCoordinators.getEmail());
			...
			newPgrmCoordinator
					.setName((null == user.getName() || user.getName().isEmpty() == true) ? (null == pgrmCoordinators
							.getName() || pgrmCoordinators.getName().isEmpty() == true) ? "" : pgrmCoordinators
			...
			try {
				...
				try {
					coordinatorAccessControlMail.sendMailToNewSecondaryCoordinator(user.getFirst_name(),
							pgrmCoordinators.getEmail(), pgrmCoordinators.getEventId());
					PMPMailLog pmpMailLog = new PMPMailLog(String.valueOf(program.getProgramId()),
							pgrmCoordinators.getEmail(), EmailLogConstants.MAIL_TO_SECONDARY_COORDINATOR,
				...
				CoordinatorAccessControlSuccessResponse cacSResponse = new CoordinatorAccessControlSuccessResponse(
						ErrorConstants.STATUS_SUCCESS, CoordinatorAccessControlConstants.APPROVER_SUCCESS_RESPONSE
								+ pgrmCoordinators.getName() + " as a coordinator for your event "
								+ pgrmCoordinators.getEventId());
				try {
					accessLog.setStatus(ErrorConstants.STATUS_SUCCESS);
					accessLog.setErrorMessage("");
					accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
