...
public class CoordinatorAccessControlServiceImpl implements CoordinatorAccessControlService {
	 ...
	 * 
	 * @param autoGeneratedEventId to get the program Id for a particular program.
	 * @param userEmail email of the requested person i.e who is raising this 
	 * request to access other than his own event.
	 * @param accessLog to register the log details into our database.
	...
	public CoordinatorAccessControlResponse addSecondaryCoordinatorRequest(String autoGeneratedEventId,String userEmail,PMPAPIAccessLog accessLog) {
		...
		if(program.getCoordinatorEmail().equalsIgnoreCase(userEmail)){
			CoordinatorAccessControlErrorResponse cacEResponse =
					new CoordinatorAccessControlErrorResponse(ErrorConstants.STATUS_FAILED,CoordinatorAccessControlConstants.REQUESTER_INVALID_SELF_REQUEST);
		...
		if(reqAlreadyRaised > 0){
			...
			CoordinatorAccessControlErrorResponse cacEResponse =
					new CoordinatorAccessControlErrorResponse(ErrorConstants.STATUS_FAILED,CoordinatorAccessControlConstants.REQUESTER_REQUEST_WAITING_FOR_APPROVAL);
		...
		if(reqAlreadyApproved > 0){
			...
			CoordinatorAccessControlErrorResponse cacEResponse =
					new CoordinatorAccessControlErrorResponse(ErrorConstants.STATUS_FAILED,CoordinatorAccessControlConstants.REQUESTER_REQUEST_ALREADY_APPROVED);
		...
		if(reqAlreadyRaised == 0){
			...
			try{
				...
				CoordinatorAccessControlSuccessResponse cacSResponse =
						new CoordinatorAccessControlSuccessResponse(ErrorConstants.STATUS_SUCCESS, CoordinatorAccessControlConstants.REQUESTER_SUCCESSFULL_REQUEST);
			...
			CoordinatorAccessControlErrorResponse cacEResponse =
					new CoordinatorAccessControlErrorResponse(ErrorConstants.STATUS_FAILED, CoordinatorAccessControlConstants.REQUESTER_FAILED_REQUEST);
			accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
			accessLog.setResponseBody(StackTraceUtils.convertPojoToJson(cacEResponse));
			apiAccessLogService.updatePmpAPIAccessLog(accessLog);
			return cacEResponse;
			...
			LOGGER.error("Error while checking request already raised for user "+userEmail +" for event "+autoGeneratedEventId);
			CoordinatorAccessControlErrorResponse cacEResponse =
	...
	/**
	 * This method is used to approve the request raised by
	 * secondary coordinators to access other's event.
	 * @param approvedBy email of the primary coordinator or the
	 * preceptor of that event.
	 * @param pgrmCoordinators Details of the secondary coordinators
	 * who wants to raise a request to access other events
	 * @param accessLog to update the PMP API access log data.
	 * @return CoordinatorAccessControlResponse, if the request is successfull,
	 * CoordinatorAccessControlSuccessResponse is returned else 
	 * CoordinatorAccessControlErrorResponse is returned.
	 */
	...
	public CoordinatorAccessControlResponse approveSecondaryCoordinatorRequest(String approvedBy, ProgramCoordinators pgrmCoordinators,PMPAPIAccessLog accessLog) {
		...
		try{
			...
			CoordinatorAccessControlErrorResponse cacEResponse =
					new CoordinatorAccessControlErrorResponse(ErrorConstants.STATUS_FAILED, CoordinatorAccessControlConstants.REQUESTER_INVALID_EMAIL_ADDRESS);
			accessLog.setErrorMessage(StackTraceUtils.convertStackTracetoString(ex));
			accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
			accessLog.setResponseBody(StackTraceUtils.convertPojoToJson(cacEResponse));
			apiAccessLogService.updatePmpAPIAccessLog(accessLog);
		...
		try{
			approvedStatus = coordntrAccssCntrlRepo.approveSecondaryCoordinatorRequest(/*program.getProgramId()*/pgrmCoordinators.getProgramId(),approvedBy,pgrmCoordinators.getEmail());
		}catch(DataAccessException daex){
		...
		if(approvedStatus > 0){
			...
			try{
				...
				CoordinatorAccessControlSuccessResponse cacSResponse =
						new CoordinatorAccessControlSuccessResponse(ErrorConstants.STATUS_SUCCESS, CoordinatorAccessControlConstants.APPROVER_SUCCESS_RESPONSE 
	...
	/**
	 * This method is used to get the list of secondary coordinators 
	 * who have raised a request to access other event details.
	 ...
	 */
	@Override
	public ResponseEntity<?> getListOfSecondaryCoordinatorRequests(String autoGeneratedEventId, PMPAPIAccessLog accessLog) {
		...
		List<SecondaryCoordinatorRequest> listOfSecondaryCoordinators = coordntrAccssCntrlRepo.getListOfRequests(program.getProgramId());
		accessLog.setStatus(ErrorConstants.STATUS_SUCCESS);
		accessLog.setErrorMessage("");
		accessLog.setTotalResponseTime(DateUtils.getCurrentTimeInMilliSec());
								...
								if (PMPConstants.CREATED_SOURCE_EXCEL.equalsIgnoreCase(source)) {
									...
									if (null == program.getCoordinatorEmail()
										...
										Runnable task = new Runnable() {
											...
											public void run() {
												try {
													coordinatorAccessControlMail
															.sendMailToPreceptorToUpdateCoordinatorEmailID(coordinator);
													System.out
															.println("Need to send mail to preceptor to update the coordinator email in dashboard.");
												} catch (Exception ex) {
										...
										Runnable task = new Runnable() {
											...
											public void run() {
												try {
													coordinatorAccessControlMail
															.sendMailToPreceptorandCoordinatorToCreateProfileAndAccessDashboard(coordinator);
													System.out
															.println("Need to send mail to preceptor and coordinator regd. profile creation and access link to dashboard.");
												} catch (Exception ex) {
													LOGGER.error("Error while sending email to the coordinator - {} ",
															program.getCoordinatorEmail());
												}
