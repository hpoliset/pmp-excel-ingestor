...
public class CoordinatorAccessControlServiceImpl implements CoordinatorAccessControlService {
	...
	public CoordinatorAccessControlResponse addSecondaryCoordinatorRequest(String autoGeneratedEventId,
			String userEmail,String secondaryCoordinatorNotes, PMPAPIAccessLog accessLog) {
		...
		if (reqAlreadyRaised == 0) {
			...
			try {
				...
				User uploaderUser = userRepository.getUserMailWithId(program.getUserId());
				//send mail to primary coordinator
				Runnable task = new Runnable() {
					@Override
					public void run() {
						try {
							coordinatorAccessControlMail.sendApprovalMailToPrimaryCoordinator(program,user,autoGeneratedEventId,secondaryCoordinatorNotes,uploaderUser.getEmail());
						} catch (Exception ex) {
