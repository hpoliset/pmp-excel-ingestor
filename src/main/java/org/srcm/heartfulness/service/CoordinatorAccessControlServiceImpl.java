...
public class CoordinatorAccessControlServiceImpl implements CoordinatorAccessControlService {
	...
	public CoordinatorAccessControlResponse addSecondaryCoordinatorRequest(String autoGeneratedEventId,String userEmail,PMPAPIAccessLog accessLog) {
		...
		if(reqAlreadyRaised == 0){
			...
			try{
				...
				try{
					if(null != pgrmCoodDetails.getEmail()){
						coordinatorAccessControlMail.sendRequestMailToCoordinatorAndPreceptor(null == pgrmCoodDetails.getName() ? "":pgrmCoodDetails.getName(),pgrmCoodDetails.getEmail(),autoGeneratedEventId);
					}
					PMPMailLog pmpMailLog = new PMPMailLog(String.valueOf(program.getProgramId()), null == pgrmCoodDetails.getEmail() ? "":pgrmCoodDetails.getEmail(),
	...
	public CoordinatorAccessControlResponse approveSecondaryCoordinatorRequest(String approvedBy, ProgramCoordinators pgrmCoordinators,PMPAPIAccessLog accessLog) {
		...
		try{
			approvedStatus = coordntrAccssCntrlRepo.approveSecondaryCoordinatorRequest(program.getProgramId(),approvedBy,pgrmCoordinators.getEmail());
		}catch(DataAccessException daex){
		...
		if(approvedStatus > 0){
			...
			newPgrmCoordinator.setName((null == user.getName() || user.getName().isEmpty() == true) ? 
			...
			try{
				...
				try{
					coordinatorAccessControlMail.sendMailToNewSecondaryCoordinator(user.getFirst_name(),pgrmCoordinators.getEmail(),pgrmCoordinators.getEventId());
					PMPMailLog pmpMailLog = new PMPMailLog(String.valueOf(program.getProgramId()), pgrmCoordinators.getEmail(),
							EmailLogConstants.MAIL_TO_SECONDARY_COORDINATOR, EmailLogConstants.STATUS_SUCCESS, null);
					try{
						mailLogRepository.createMailLog(pmpMailLog);
					}catch(Exception ex){
						LOGGER.error("Failed to create mail log table ",ex);
					...
					LOGGER.error("Failed to send request mail to " + program.getCoordinatorEmail());
					PMPMailLog pmpMailLog = new PMPMailLog(String.valueOf(program.getProgramId()), pgrmCoordinators.getEmail(),
							EmailLogConstants.MAIL_TO_SECONDARY_COORDINATOR, EmailLogConstants.STATUS_FAILED, StackTraceUtils.convertStackTracetoString(ex));
					try{
						mailLogRepository.createMailLog(pmpMailLog);
					}catch(Exception e){
						LOGGER.error("Failed to update mail log table ",e);
