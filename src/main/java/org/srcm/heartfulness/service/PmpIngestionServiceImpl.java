...
public class PmpIngestionServiceImpl implements PmpIngestionService {
			...
			if (version != version.INVALID) {
				...
				if (!errorResponse.isEmpty()) {
					...
					try {
						...
						int id = 0;
						try {
							accessLog = new PMPAPIAccessLog(null, null, "EXCEL UPLOAD",
									DateUtils.getCurrentTimeInMilliSec(), null, ErrorConstants.STATUS_FAILED, null,
									StackTraceUtils.convertPojoToJson(program.getAutoGeneratedEventId()), null);
						...
						if(null == isCoordinatorEmailIdvalid){
							...
							String isPreceptorIdValid=programService.validatePreceptorIDCardNumber(program, id);
	...
	private String validateCoordinatorEmailID(Program program, int id) {
		...
		if(null != program.getCoordinatorEmail() && !program.getCoordinatorEmail().isEmpty()){
			try {
				...
				try {
					fetchEwelcomeIDAPIAccessLogDetails = new PMPAPIAccessLogDetails(id, EndpointConstants.ABHYASI_INFO_URI+"?email_exact="+program.getCoordinatorEmail(),
							DateUtils.getCurrentTimeInMilliSec(), null, ErrorConstants.STATUS_FAILED, null,
							StackTraceUtils.convertPojoToJson("Request: Coordinator Email:" + program.getCoordinatorEmail()+", Coordinator Name:" + program.getCoordinatorName()));
				...
				if (abhyasiResult.getUserProfile().length > 0) {
					...
					if (null == userProfile) {
						...
						ProgramCoordinators programCoordinators=new ProgramCoordinators(program.getProgramId(), user.getId(), program.getCoordinatorName(), program.getCoordinatorEmail(), 1);
				...
				LOGGER.error("HTTP CLIENT ERROR : Error While Validating email of coordinator. NAME: {}, EMAIL: {}, EXCEPTION: {} ",program.getCoordinatorName(),program.getCoordinatorEmail(),StackTraceUtils.convertStackTracetoString(e));
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
				...
				LOGGER.error("JSONPARSE/JSONMAPPING ERROR : Error While Validating email of coordinator. NAME: {}, EMAIL: {}, EXCEPTION: {} ",program.getCoordinatorName(),program.getCoordinatorEmail(),StackTraceUtils.convertStackTracetoString(e));
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
					fetchEwelcomeIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_FAILED);
					fetchEwelcomeIDAPIAccessLogDetails.setErrorMessage(StackTraceUtils.convertStackTracetoString(e));
				...
				LOGGER.error("Exception : Error While Validating email of coordinator. NAME: {}, EMAIL: {}, EXCEPTION: {} ",program.getCoordinatorName(),program.getCoordinatorEmail(),StackTraceUtils.convertStackTracetoString(e));
				try {
					fetchEwelcomeIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
					fetchEwelcomeIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_FAILED);
					fetchEwelcomeIDAPIAccessLogDetails.setErrorMessage(StackTraceUtils.convertStackTracetoString(e));
	...
	private void validatePreceptorIDAsyncronously(Program program, int id) {
		Runnable task = new Runnable() {
            @Override
             public void run() {
                 try {
                	 validatePreceptorID(program,id);
                 } catch (Exception ex) {
	 ...
	 * @param program
	 */
	private void validatePreceptorID(Program program, int id) {
		PMPAPIAccessLogDetails validatePreceptorIDAPIAccessLogDetails = null;
		LOGGER.info("Inserting PMP API log details in table");
		try {
			validatePreceptorIDAPIAccessLogDetails = new PMPAPIAccessLogDetails(id, EndpointConstants.ABHYASI_INFO_URI,
					DateUtils.getCurrentTimeInMilliSec(), null, ErrorConstants.STATUS_FAILED, null,
					"PreceptorId : " + program.getPreceptorIdCardNumber()
					+ " ,Event ID : " + program.getAutoGeneratedEventId());
			apiAccessLogService.createPmpAPIAccesslogDetails(validatePreceptorIDAPIAccessLogDetails);
		} catch (Exception e) {
			LOGGER.error("Exception while inserting PMP API log details in table : {} ",
					StackTraceUtils.convertPojoToJson(e));
		}
		try {
			...
			if (null != isValid) {
				...
				try {
					if (null != validatePreceptorIDAPIAccessLogDetails) {
						validatePreceptorIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_FAILED);
						validatePreceptorIDAPIAccessLogDetails.setResponseBody(StackTraceUtils.convertPojoToJson(isValid));
						validatePreceptorIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
						apiAccessLogService.updatePmpAPIAccesslogDetails(validatePreceptorIDAPIAccessLogDetails);
					}
				} catch (Exception e) {
					LOGGER.error("Exception while inserting PMP API log details in table : {} ",
							StackTraceUtils.convertPojoToJson(e));
				...
				try {
					if (null != validatePreceptorIDAPIAccessLogDetails) {
						validatePreceptorIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_SUCCESS);
						validatePreceptorIDAPIAccessLogDetails.setResponseBody("Valid PreceptorID");
						validatePreceptorIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
						apiAccessLogService.updatePmpAPIAccesslogDetails(validatePreceptorIDAPIAccessLogDetails);
					}
				} catch (Exception e) {
			...
			try {
				if (null != validatePreceptorIDAPIAccessLogDetails) {
					validatePreceptorIDAPIAccessLogDetails.setStatus(ErrorConstants.STATUS_FAILED);
					validatePreceptorIDAPIAccessLogDetails.setResponseBody(StackTraceUtils.convertPojoToJson("Error while validating preceptor ID"));
					validatePreceptorIDAPIAccessLogDetails.setResponseTime(DateUtils.getCurrentTimeInMilliSec());
					apiAccessLogService.updatePmpAPIAccesslogDetails(validatePreceptorIDAPIAccessLogDetails);
				}
			} catch (Exception e) {
