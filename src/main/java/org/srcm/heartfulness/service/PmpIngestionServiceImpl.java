...
public class PmpIngestionServiceImpl implements PmpIngestionService {
	...
	private void validatePreceptorID(Program program) {
		...
		try {
			accessLog = new PMPAPIAccessLog(program.getCoordinatorEmail(), "EXCEL UPLOAD", EndpointConstants.ABHYASI_INFO_URI, DateUtils.getCurrentTimeInMilliSec(), null,
			...
			LOGGER.info("Exception while inserting PMP API log details in table : {} ",
