...
public class ParticipantRepositoryImpl implements ParticipantRepository {
	...
	@Override
	public Map<Program, List<Participant>> getGeneratedEwelcomeIdDetails() {
		return this.jdbcTemplate
				.query("SELECT p.program_start_date,p.auto_generated_event_id,p.program_id,p.preceptor_id_card_number,p.first_sitting_by,p.coordinator_email,pr.print_name"
						+",pr.first_name,pr.date_of_birth,pr.email,pr.third_sitting_date,pr.third_sitting,pr.city,pr.state,pr.country,pr.welcome_card_number,"
						+"pr.welcome_card_date,pr.id,pr.mobile_phone,pr.introduction_date,pr.address_line1,pr.address_line2,"
						+"pr.ewelcome_id_state,pr.ewelcome_id_remarks,pr.seqId FROM program p,participant pr"
						+ " WHERE p.program_id = pr.program_id AND pr.create_time <= CURRENT_TIMESTAMP"
						+ " AND pr.ewelcome_id_state = 'T' AND ("
						+ "pr.welcome_card_number IS NULL OR pr.welcome_card_number = '')", new Object[] {},
						new ResultSetExtractor<Map<Program, List<Participant>>>() {
							@Override
							public Map<Program, List<Participant>> extractData(ResultSet resultSet)
									throws SQLException, DataAccessException {
								Map<Program, List<Participant>> eWelcomeIdDetails = new HashMap<Program, List<Participant>>();
								while (resultSet.next()) {
									Program program = new Program();
									Participant participant = new Participant();
									program.setProgramStartDate(resultSet.getDate(1));
									program.setAutoGeneratedEventId(resultSet.getString(2));
									program.setProgramId(resultSet.getInt(3));
									program.setPreceptorIdCardNumber(resultSet.getString(4));
									program.setFirstSittingBy(resultSet.getInt(5));
									program.setCoordinatorEmail(resultSet.getString(6));
									participant.setProgramId(resultSet.getInt(3));
									participant.setPrintName(resultSet.getString(7));
									participant.setFirstName(resultSet.getString(8));
									participant.setDateOfBirth(resultSet.getDate(9));
									participant.setEmail(resultSet.getString(10));
									participant.setThirdSittingDate(resultSet.getDate(11));
									participant.setThirdSitting(resultSet.getInt(12));
									participant.setCity(resultSet.getString(13));
									participant.setState(resultSet.getString(14));
									participant.setCountry(resultSet.getString(15));
									participant.setWelcomeCardNumber(resultSet.getString(16));
									participant.setWelcomeCardDate(resultSet.getDate(17));
									participant.setId(resultSet.getInt(18));
									participant.setMobilePhone(resultSet.getString(19));
									participant.setIntroductionDate(resultSet.getDate(20));
									participant.setAddressLine1(resultSet.getString(21));
									participant.setAddressLine2(resultSet.getString(22));
									participant.setEwelcomeIdState(resultSet.getString(23));
									participant.setEwelcomeIdRemarks(resultSet.getString(24));
									participant.setSeqId(resultSet.getString(25));
									if (eWelcomeIdDetails.containsKey(program)) {
										eWelcomeIdDetails.get(program).add(participant);
									} else {
										List<Participant> participants = new ArrayList<Participant>();
										participants.add(participant);
										eWelcomeIdDetails.put(program, participants);
									}
								}
	...
	public void UpdateParticipantEwelcomeIDDetails(Participant participant) {
		...
		if(0 != participant.getId()){
			this.namedParameterJdbcTemplate.update("UPDATE participant SET " + "introduction_date=:introductionDate,"
					...
					+ "is_ewelcome_id_informed=:isEwelcomeIdInformed, " + "introduced=:introduced, "
					+ "ewelcome_id_state=:ewelcomeIdState, " + "ewelcome_id_remarks=:ewelcomeIdRemarks " + "WHERE id=:id", params);
		}
