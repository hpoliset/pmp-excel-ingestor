...
public class ParticipantRepositoryImpl implements ParticipantRepository {
		...
		if (participant.getId() == 0) {
			...
			if (participantId > 0) {
				...
				if (null != seqId && !seqId.isEmpty()) {
					...
					participant.setSeqId(checkExistanceOfAutoGeneratedSeqId(SmsUtil.generateFourDigitPIN()));
				}
			} else {
				participant.setSeqId(checkExistanceOfAutoGeneratedSeqId(SmsUtil.generateFourDigitPIN()));
			}
	...
	@Override
	public String checkExistanceOfAutoGeneratedSeqId(String generatedSeqId) {
		String eventIdExistence = this.jdbcTemplate.query(
				"SELECT seqId from participant WHERE seqId=?",
				new Object[] { generatedSeqId }, new ResultSetExtractor<String>() {
					@Override
					public String extractData(ResultSet resultSet) throws SQLException, DataAccessException {
						if (resultSet.next()) {
							return resultSet.getString(1);
		...
		if (null != eventIdExistence) {
			...
			return checkExistanceOfAutoGeneratedSeqId(generatedSeqId);
		} else {
