...
public class CoordinatorAccessControlRepositoryImpl implements CoordinatorAccessControlRepository {
	 ...
	 *  generatedRequestId exists or not 
	 * 
	 * @param autoGnrtdRequestId Id for a particular Event access request.
	...
	private int checkAutoGnrtdRequestIdAlreadyExists(String autoGnrtdRequestId){
		...
		try{
			requestIdCount = this.jdbcTemplate.query(
					"SELECT count(request_id) from event_access_request where auto_generated_request_id=?", new Object[] {autoGnrtdRequestId},
			...
			LOGGER.error("DAE Failed to get count of already generated session id for auto generated session id "+autoGnrtdRequestId);
			return 0;
		} catch(Exception ex){
			LOGGER.error("Ex Failed to get count of already generated session id for auto generated session id "+autoGnrtdRequestId);
			return -1;
		}
	 ...
	 */
	@Override
	public List<SecondaryCoordinatorRequest> getListOfRequests(int programId) {
		
		List<SecondaryCoordinatorRequest> secondaryCoordinatorList = new ArrayList<SecondaryCoordinatorRequest>();
		
		secondaryCoordinatorList = this.jdbcTemplate.query("SELECT auto_generated_request_id,status, "
				+ "requested_by,request_time "
				+ "FROM event_access_request "
				+ "WHERE program_id = ?",
				new Object[] {programId}, 
				new ResultSetExtractor<List<SecondaryCoordinatorRequest>>() {
					@Override
					public List<SecondaryCoordinatorRequest> extractData(ResultSet resultSet) throws SQLException, DataAccessException {
						...
						while (resultSet.next()) {
							...
							request.setAutoGeneratedRequestId(resultSet.getString(1));
							request.setStatus(resultSet.getString(2));
							request.setRequestedBy(resultSet.getString(3));
							request.setRequestTime(resultSet.getDate(4));
