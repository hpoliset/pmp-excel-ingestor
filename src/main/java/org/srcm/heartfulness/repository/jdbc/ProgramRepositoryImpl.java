...
public class ProgramRepositoryImpl implements ProgramRepository {
	...
	public void save(Program program) {
		...
		if (program.getProgramId() == 0) {
			...
			if (programId > 0) {
				...
				if (autoGeneratedEventId != null && !autoGeneratedEventId.isEmpty()) {
					...
					program.setAutoGeneratedEventId(ExpressionConstants.EVENT_ID_PREFIX
				...
				if (autoGeneratedIntroId != null && !autoGeneratedIntroId.isEmpty()) {
					...
					program.setAutoGeneratedIntroId(ExpressionConstants.INTRO_ID_PREFIX
							+ SmsUtil.generateRandomNumber(7));
				}

			} else {

				program.setAutoGeneratedEventId(ExpressionConstants.EVENT_ID_PREFIX + SmsUtil.generateRandomNumber(6));
				program.setAutoGeneratedIntroId(ExpressionConstants.INTRO_ID_PREFIX + SmsUtil.generateRandomNumber(7));
			}
		...
		if (program.getProgramId() == 0) {
			...
			this.namedParameterJdbcTemplate.update("UPDATE program SET " + "program_hash_code=:programHashCode, "
					...
					+ "srcm_group=:srcmGroup, " + "first_sitting_by=:firstSittingBy " + "WHERE program_id=:programId",
					parameterSource);
		}
	...
	/*
	 * @Override public List<Program> getEventsByEmail(String email, boolean
	 * isAdmin) { List<Program> program = null; StringBuilder whereCondition =
	 * new StringBuilder(""); Map<String, Object> params = new HashMap<>();
	 * params.put("coordinator_email", email); SqlParameterSource
	 * sqlParameterSource = new MapSqlParameterSource(params);
	 * 
	 * if (!isAdmin) {
	 * whereCondition.append("coordinator_email=:coordinator_email"); } program
	 ...
	 * " FROM program" + (whereCondition.length() > 0 ? " WHERE " +
	 * whereCondition : ""), sqlParameterSource,
	 * BeanPropertyRowMapper.newInstance(Program.class)); return program; }
	 */
	@Override
	...
	public Program saveProgram(Program program) {
		...
		if (program.getProgramId() == 0) {
			...
			this.namedParameterJdbcTemplate.update("UPDATE program SET " + "program_hash_code=:programHashCode, "
					...
					+ "welcome_card_signer_id_card_number=:welcomeCardSignerIdCardNumber," + "remarks=:remarks, "
					+ "auto_generated_event_id=:autoGeneratedEventId, "
					+ "auto_generated_intro_id=:autoGeneratedIntroId, " + "first_sitting_by=:firstSittingBy, "
					+ "abyasi_ref_no=:abyasiRefNo " + "WHERE program_id=:programId", parameterSource);
		}
		return program;
	}
	...
	/*
	 * @Override public List<Program> searchEvents(SearchRequest searchRequest)
	 * { List<Program> program = null; StringBuilder whereCondition = new
	 * StringBuilder(""); StringBuilder orderBy = new StringBuilder("");
	 * Map<String, Object> params = new HashMap<>(); if
	 * (!("ALL".equals(searchRequest.getSearchField())) && null !=
	 * searchRequest.getSearchField() &&
	 * !searchRequest.getSearchField().isEmpty()) { if (null !=
	 * searchRequest.getSearchText() &&
	 * !searchRequest.getSearchText().isEmpty()) {
	 * whereCondition.append(whereCondition.length() > 0 ? " and " +
	 * searchRequest.getSearchField() + " LIKE '%" +
	 * searchRequest.getSearchText() + "%'" : searchRequest.getSearchField() +
	 * " LIKE '%" + searchRequest.getSearchText() + "%'"); } } if
	 * ((searchRequest.getDateFrom() != null &&
	 * !searchRequest.getDateFrom().isEmpty())) { try {
	 * whereCondition.append(whereCondition.length() > 0 ?
	 * " and program_start_date >=:program_start_date " :
	 * " program_start_date >=:program_start_date ");
	 * params.put("program_start_date",
	 * DateUtils.parseToSqlDate(searchRequest.getDateFrom())); } catch
	 * (ParseException e) { LOGGER.error("Error While converting date", e); } }
	 * 
	 * if (searchRequest.getDateTo() != null &&
	 * !searchRequest.getDateTo().isEmpty()) { try { whereCondition
	 * .append(whereCondition.length() > 0 ?
	 * " and CASE WHEN program_end_date IS NOT NULL THEN program_end_date <=:program_end_date ELSE TRUE END "
	 * :
	 * " CASE WHEN program_end_date IS NOT NULL THEN program_end_date <=:program_end_date ELSE TRUE END "
	 * ); params.put("program_end_date",
	 * DateUtils.parseToSqlDate(searchRequest.getDateTo())); } catch
	 * (ParseException e) { LOGGER.error("Error While converting date", e); } }
	 * if (null != searchRequest.getSortBy() &&
	 * !searchRequest.getSortBy().isEmpty()) { orderBy.append(orderBy.length() >
	 * 0 ? ", " + searchRequest.getSortBy() : searchRequest.getSortBy()); if
	 * (null != searchRequest.getSortDirection() &&
	 * !searchRequest.getSortDirection().isEmpty()) {
	 * orderBy.append(searchRequest.getSortDirection().equalsIgnoreCase("0") ?
	 * " asc" : " desc"); } }
	 * 
	 * program = this.namedParameterJdbcTemplate.query(
	 ...
	 * "preceptor_id_card_number" + " FROM program" + (whereCondition.length() >
	 * 0 ? " WHERE " + whereCondition : "") + (orderBy.length() > 0 ?
	...
	public int getPgrmCountBySrchParams(SearchRequest searchRequest, String userEmail, boolean isAdmin) {
		...
		return this.namedParameterJdbcTemplate.queryForObject("SELECT COUNT(program_id) FROM program "
				+ (whereCondition.length() > 0 ? " WHERE " + whereCondition : "")
				+ (isAdminCondition.length() > 0 ? isAdminCondition : "")
	...
	/*
	 * (non-Javadoc)
	 * @see org.srcm.heartfulness.repository.ProgramRepository#isEventCoordinatorExistsWithUserEmailId(java.lang.String)
	 */
	@Override
	...
	public int getProgramCountWithUserRoleAndEmailId(String email, String role) {
		...
		if (!role.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN)) {
			whereCondition.append(whereCondition.length() > 0 ?" and (program_channel NOT LIKE '%G-Connect%' OR coordinator_email=:coordinator_email) ": "(program_channel NOT LIKE '%G-Connect%' OR coordinator_email=:coordinator_email)");
			params.put("coordinator_email", email);
	...
	public List<Program> getEventsByEmailAndRole(String email, String role, int offset, int pageSize) {
		...
		for (Integer secondaryprogramId : secondaryprogramIds) {
			params = new HashMap<>();
			params.put("program_id", secondaryprogramId);
			Program secondaryProgram=this.namedParameterJdbcTemplate.queryForObject(
					"SELECT program_id,program_channel,program_start_date,program_end_date,"
							+ "coordinator_name,coordinator_email,coordinator_mobile,event_place,"
							+ "event_city,event_state,event_country,organization_department,"
							+ "organization_name,organization_web_site,organization_contact_name,"
							...
							+ "preceptor_id_card_number,welcome_card_signed_by_name,welcome_card_signer_Id_card_number,"
							+ "remarks,auto_generated_event_id,auto_generated_intro_id" + " FROM program"
							+ " WHERE program_id=:program_id", params,BeanPropertyRowMapper.newInstance(Program.class));
	...
	@Override
	public List<Program> searchEventsWithUserRoleAndEmailId(SearchRequest searchRequest, String email, String role,
			int offset) {
		List<Program> program = null;
		StringBuilder whereCondition = new StringBuilder("");
		StringBuilder isAdminCondition = new StringBuilder("");
