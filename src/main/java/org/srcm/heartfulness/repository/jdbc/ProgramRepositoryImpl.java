...
public class ProgramRepositoryImpl implements ProgramRepository {
	...
	public void saveWithProgramName(Program program) {
		...
		if (program.getProgramId() == 0) {
			...
			.update("UPDATE program SET " + "program_hash_code=:programHashCode, "
					...
					+ "auto_generated_intro_id=:autoGeneratedIntroId, " + "first_sitting_by=:firstSittingBy " + "WHERE program_id=:programId",
					parameterSource);
		}
	...
	public void updatePreceptorDetails(Program program) {
		System.out.println( program.getPreceptorName() + "  "+ program.getAbyasiRefNo() +"  "+ program.getAbyasiRefNo() +"   "+program.getFirstSittingBy()+"   "+program.getProgramId());
		Map<String, Object> params = new HashMap<>();
		...
		if(0 != program.getProgramId()){
			this.namedParameterJdbcTemplate.update("UPDATE program SET " + "preceptor_name=:preceptorName,"
					+ "abyasi_ref_no=:abyasiRefNo, "
					+ "first_sitting_by=:firstSittingBy " + "WHERE program_id=:programId",
					params);
		}
