...
public class ProgramRepositoryImpl implements ProgramRepository {
	...
	public ProgramRepositoryImpl(DataSource dataSource, ParticipantRepository participantRepository) {
		...
		this.insertProgram = new SimpleJdbcInsert(dataSource).withTableName("program")
		...
		this.insertDeletedParticipants = new SimpleJdbcInsert(dataSource).withTableName("deleted_participants")
				.usingGeneratedKeyColumns("id");
		this.insertProgramPermissionLetter = new SimpleJdbcInsert(dataSource)
				.withTableName("program_permission_letters").usingGeneratedKeyColumns("permission_letter_id");
		this.namedParameterJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
		this.jdbcTemplate = new JdbcTemplate(dataSource);
		this.insertCoordinatorHistory = new SimpleJdbcInsert(dataSource).withTableName("coordinator_history")
				.usingGeneratedKeyColumns("id");
		this.insertUploadedFiles = new SimpleJdbcInsert(dataSource).withTableName("uploaded_files")
				.usingGeneratedKeyColumns("id");
		this.insertProgramTestimonial = new SimpleJdbcInsert(dataSource).withTableName("program_testimonials")
				.usingGeneratedKeyColumns("testimonial_id");
	}
	 ...
	 * findUpdatedProgramIdsSince (java.util.Date;)
	 */
	@Override
	...
	public void save(Program program) {
		...
		if (program.getProgramId() == 0) {
			...
			if (programId > 0) {
				...
				if (autoGeneratedEventId != null && !autoGeneratedEventId.isEmpty()) {
					...
					program.setAutoGeneratedEventId(checkExistanceOfAutoGeneratedEventId(
							ExpressionConstants.EVENT_ID_PREFIX + SmsUtil.generateRandomNumber(6)));
				}

				if (autoGeneratedIntroId != null && !autoGeneratedIntroId.isEmpty()) {
					program.setAutoGeneratedIntroId(autoGeneratedIntroId);
				} else {
					program.setAutoGeneratedIntroId(checkExistanceOfAutoGeneratedIntroId(
							ExpressionConstants.INTRO_ID_PREFIX + SmsUtil.generateRandomNumber(7)));
				}
		...
		if (program.getProgramId() == 0) {
			...
			saveCoordinatorHistory(new CoordinatorHistory(newId.intValue(), program.getCoordinatorName(),
					program.getCoordinatorEmail(),
					null != program.getCoordinatorAbhyasiId() ? program.getCoordinatorAbhyasiId() : null));
			program.setStatusDescription(DashboardConstants.EVENT_ID + program.getAutoGeneratedEventId() + DashboardConstants.EVENT_CREATION_STATUS);

		} else {
			// TODO: Need to deal with Hashcode.
			this.namedParameterJdbcTemplate.update(
					"UPDATE program SET " + "program_hash_code=:programHashCode, " + "program_name=:programName, "
							+ "program_channel=:programChannel, " + "program_start_date=:programStartDate, "
							+ "program_end_date=:programEndDate," + "coordinator_name=:coordinatorName,"
							+ "coordinator_email=:coordinatorEmail," + "coordinator_mobile=:coordinatorMobile,"
							+ "event_place=:eventPlace," + "event_city=:eventCity," + "event_state=:eventState,"
							+ "event_country=:eventCountry," + "organization_department=:organizationDepartment,"
							...
							+ "srcm_group=:srcmGroup, " + "first_sitting_by=:firstSittingBy, "
							+ "coordinator_abhyasi_id=:coordinatorAbhyasiId, " + "program_zone=:programZone, "
							+ "program_center=:programCenter, " + "organization_batch_no=:organizationBatchNo, "
							+ "organization_city=:organizationCity, " + "organization_location=:organizationLocation, "
							+ "organization_full_address=:organizationFullAddress, "
							+ "organization_decision_maker_name=:organizationDecisionMakerName, "
							+ "organization_decision_maker_email=:organizationDecisionMakerEmail, "
							...
							+ "jira_issue_number=:jiraIssueNumber, " + "senders_email_address=:sendersEmailAddress, "
			...
			program.setStatusDescription(DashboardConstants.EVENT_ID + program.getAutoGeneratedEventId() + DashboardConstants.EVENT_UPDATION_STATUS);
		}
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * findByAutoGeneratedEventId (java.lang.String)
	 */
	@Override
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * findByAutoGeneratedIntroId (java.lang.String)
	 */
	@Override
	...
	public Program getEventById(int id) {
		...
		program = this.namedParameterJdbcTemplate.queryForObject(
				"SELECT program_id,program_channel,program_start_date,program_end_date,"
						...
						+ " FROM program WHERE program_id=:id" + " OR auto_generated_event_id=:id",
						params, BeanPropertyRowMapper.newInstance(Program.class));
		List<Participant> participants = null;
		// get the participantList
		if (program != null && program.getProgramId() > 0) {
	 ...
	 * 
	 * @see
	 * org.srcm.heartfulness.repository.ProgramRepository#getParticipantList(
	...
	public Program saveProgram(Program program) {
		...
		if (program.getProgramId() == 0) {
			...
			this.namedParameterJdbcTemplate.update("UPDATE program SET " + "program_hash_code=:programHashCode, "
					...
					+ "welcome_card_signer_id_card_number=:welcomeCardSignerIdCardNumber,"
					+ "abyasi_ref_no=:abyasiRefNo, " + "auto_generated_event_id=:autoGeneratedEventId, "
					+ "auto_generated_intro_id=:autoGeneratedIntroId, "
					+ "coordinator_abhyasi_id=:coordinatorAbhyasiId, "
					+ "coordinator_permission_letter_path=:coordinatorPermissionLetterPath, "
					+ "program_zone=:programZone, " + "program_center=:programCenter, "
					+ "organization_batch_no=:organizationBatchNo, " + "organization_city=:organizationCity, "
	...
	public List<String> getProgramIdAndCreatedSourceByEventId(String eventId) {
		...
		try{
			...
			details = this.jdbcTemplate.query("SELECT program_id,created_source from program where auto_generated_event_id=?",
					new Object[] { eventId }, new ResultSetExtractor<List<String>>() {
				@Override
				public List<String> extractData(ResultSet resultSet) throws SQLException, DataAccessException {
					List<String> programDetails = new ArrayList<String>();
					if (resultSet.next()) {
						programDetails.add(String.valueOf(resultSet.getInt(1)));
						programDetails.add(resultSet.getString(2));
					}
			...
			LOGGER.error("Unable to fetch program Id and created source using auto_generated_program_id {}",eventId);
		}
		return details;
	}
	...
	public int getEventCountByCategory(String email, boolean isAdmin, String eventCategory) {
		...
		int count = this.namedParameterJdbcTemplate
				.queryForObject(
						"SELECT COUNT(program_channel)" + " FROM program"
								+ (whereCondition.length() > 0
										? " WHERE (program_channel=:program_channel) " + whereCondition : ""),
								params, Integer.class);
		return count;
	}
	...
	public int getMiscellaneousEventsByEmail(String email, boolean isAdmin, List<String> eventcategories) {
		...
		int count = this.namedParameterJdbcTemplate.queryForObject("SELECT COUNT(program_channel)" + " FROM program"
				+ (whereCondition.length() > 0
						? " WHERE ( (program_channel NOT IN (SELECT distinct `name` FROM `channel`)) "
						+ "AND ( program_channel IS NOT NULL " + "AND program_channel!='') ) " + whereCondition
						: ""),
				params, Integer.class);
		return count;
	}
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * findParticipantBySeqIdAndRole(java.lang.String, int, java.util.List,
	 * java.lang.String)
	 */
	@Override
	public Participant findParticipantBySeqIdAndRole(String seqId, int programId, List<String> emailList,String userRole) {
		 ...
		 * String userRole = this.jdbcTemplate.query(
		 * "SELECT role from user WHERE email=? ", new Object[] { mail }, new
		 * ResultSetExtractor<String>() {
		 * 
		 * @Override public String extractData(ResultSet resultSet) throws
		 * SQLException, DataAccessException { if (resultSet.next()) { return
		 * resultSet.getString(1); } return null; } });
		...
		if (emailList.size() == 1) {
			...
			for (int i = 0; i < emailList.size(); i++) {
				emailString.append(i != (emailList.size() - 1) ? "'" + emailList.get(i) + "'" + ","
						: "'" + emailList.get(i) + "'");
			}
		}
		...
		if (!userRole.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN)) {
			...
			if (userRole.equalsIgnoreCase(PMPConstants.LOGIN_ROLE_ADMIN)) {
				...
				whereCondition.append(
						" AND (p.coordinator_email IN(" + emailString + ") OR pc.email IN(" + emailString + ")) ");
			}
		}
		...
		if ((null != userRole && userRole.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN))
			...
			try {
				...
				participants = this.namedParameterJdbcTemplate
						.query(
								"SELECT DISTINCT pr.*"
										+ " FROM participant pr,program p LEFT JOIN program_coordinators pc ON p.program_id = pc.program_id"
										+ " WHERE seqId=:seqId AND pr.program_id=:programId AND pr.program_id=p.program_id"
				...
				LOGGER.info("SQL Exception occurred:{}",ex.getMessage());
				return null;
			}
		}
		return null;
	...
	public void updateParticipantsStatus(String participantIds, String eventId, String introduced, String userEmailID) {
		...
		this.namedParameterJdbcTemplate.update(
				"UPDATE participant SET introduced=:introduced,introduced_by=:introducedBy,introduction_date=:introducedDate WHERE program_id=:programId AND seqId=:seqId ",
				params);
	...
	public List<String> getNonCategorizedEventListByEmail(String username, boolean isAdmin) {
		...
		List<String> UncategorizedEvents = this.namedParameterJdbcTemplate.queryForList(
				"SELECT distinct program_name" + " FROM program"
						+ (whereCondition.length() > 0
								? " WHERE (program_channel IS NULL OR program_channel='') " + whereCondition : ""),
						params, String.class);
	...
	public Participant deleteParticipant(String seqId, String eventId) {
		...
		if (null != participant) {
			...
			.update("DELETE FROM `participant` WHERE program_id=:programId AND seqId=:seqId ", params);
			return participant;
		}
	...
	public void updateDeletedParticipant(Participant deletedParticipant, String deletedBy) {
		try {
			...
			if (null != newId) {
				...
				this.namedParameterJdbcTemplate
				.update("UPDATE deleted_participants SET deleted_by=:deleted_by WHERE id=:id ", params);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public int getProgramCount(String userEmail, boolean isAdmin) {
		...
		int programCount = this.namedParameterJdbcTemplate
				.queryForObject(
	...
	public void updatePreceptorDetails(Program program) {
		...
		if (0 != program.getProgramId()) {
			this.namedParameterJdbcTemplate
			.update("UPDATE program SET " + "preceptor_name=:preceptorName," + "abyasi_ref_no=:abyasiRefNo, "
					+ "first_sitting_by=:firstSittingBy " + "WHERE program_id=:programId", params);
		}
	...
	public int getProgramCountWithUserRoleAndEmailId(List<String> emailList, String role) {
		...
		if (!role.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN)) {
			...
			if (role.equalsIgnoreCase(PMPConstants.LOGIN_ROLE_ADMIN)) {
				...
				whereCondition
				.append(" (p.coordinator_email IN( " + emailString + ") OR pc.email IN(" + emailString + ")) ");
			}
		}

		int programCount = this.jdbcTemplate.queryForObject("SELECT count(DISTINCT p.program_id ) "
				+ " FROM program p LEFT JOIN program_coordinators pc " + " ON p.program_id = pc.program_id "
	...
	public List<Program> getEventsByEmailAndRole(List<String> emailList, String role, int offset, int pageSize) {
		...
		programs = this.jdbcTemplate.query(
				"SELECT DISTINCT p.program_id,p.program_channel,p.program_start_date,"
						...
						+ "FROM program p LEFT JOIN program_coordinators pc " + " ON p.program_id = pc.program_id "
						+ (whereCondition.length() > 0 ? " WHERE " + whereCondition : "")
						+ " ORDER BY p.program_start_date DESC" + (limitCondition.length() > 0 ? limitCondition : ""),
						BeanPropertyRowMapper.newInstance(Program.class));
	...
	/*@Override
	public int getPgrmCountBySrchParamsWithUserRoleAndEmailId(SearchRequest searchRequest,
			List<String> emailList String email , String role) {
		StringBuilder whereCondition = new StringBuilder("");
		...
		if (!("ALL".equals(searchRequest.getSearchField())) && null != searchRequest.getSearchField()
			...
			if (null != searchRequest.getSearchText() && !searchRequest.getSearchText().isEmpty()) {
				whereCondition.append(whereCondition.length() > 0
						? " AND p." + searchRequest.getSearchField() + " LIKE '%" + searchRequest.getSearchText() + "%'"
								: "p." + searchRequest.getSearchField() + " LIKE '%" + searchRequest.getSearchText() + "%'");
			}
		}
		...
		if (searchRequest.getDateTo() != null && !searchRequest.getDateTo().isEmpty()) {
			try {
				whereCondition.append(whereCondition.length() > 0
						? " and CASE WHEN p.program_start_date IS NOT NULL THEN p.program_start_date <=:program_end_date ELSE TRUE END "
		...
		if (!role.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN)) {
			...
			if (role.equalsIgnoreCase(PMPConstants.LOGIN_ROLE_ADMIN)) {
				whereCondition.append(whereCondition.length() > 0
						? " AND (p.program_channel NOT REGEXP ('G-Connect|G Connect|GConnect|G-Conect|G - Connect|G.CONNECT|G -CONNECT|G- connect|G-Connet|G  Connect')"
						+ " OR (p.coordinator_email IN( " + emailString + ") OR pc.email IN(" + emailString
						+ "))) "
		...
		int programCount = this.namedParameterJdbcTemplate.queryForObject("SELECT count(DISTINCT p.program_id ) "
				+ " FROM program p LEFT JOIN program_coordinators pc" + " ON p.program_id = pc.program_id "
	...
	/*@Override
	public List<Program> searchEventsWithUserRoleAndEmailId(SearchRequest searchRequest,
			List<String> emailList String email , String role, int offset) {
		...
		programs = this.namedParameterJdbcTemplate.query(
				"SELECT DISTINCT p.program_id,p.auto_generated_event_id,p.program_channel, "
						...
						+ " LIMIT " + offset + "," + searchRequest.getPageSize(),
						params, BeanPropertyRowMapper.newInstance(Program.class));
		return programs;
	}*/
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * checkExistanceOfAutoGeneratedEventId(java.lang.String)
	 */
	@Override
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * checkExistanceOfAutoGeneratedIntroId(java.lang.String)
	 */
	@Override
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * getProgramAndParticipantDetails(java.lang.String)
	 */
	@Override
	public List<String> getProgramAndParticipantDetails(String autoGeneratedEventId) {
		...
		eventPctptDetails = this.jdbcTemplate.query(
				"SELECT pgrm.program_id,pgrm.is_ewelcome_id_generation_disabled," + " pgrm.coordinator_email "
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * saveProgramPermissionLetterDetails(org.srcm.heartfulness.model.
	 * ProgramPermissionLetterdetails)
	 */
	/*
	 * @Override public void
	 * saveProgramPermissionLetterDetails(ProgramPermissionLetterdetails
	 * programPermissionLetterdetails) { BeanPropertySqlParameterSource
	 * parameterSource = new BeanPropertySqlParameterSource(
	 * programPermissionLetterdetails); if (0 ==
	 * programPermissionLetterdetails.getPermissionLetterId()) { int
	 * permissionLetterId = this.jdbcTemplate .query(
	 * "SELECT permission_letter_id FROM program_permission_letters WHERE program_id=? AND permission_letter_name=? "
	 * , new Object[] { programPermissionLetterdetails.getProgramId(),
	 * programPermissionLetterdetails.getPermissionLetterName() }, new
	 ...
	 * } else { this.namedParameterJdbcTemplate .update(
	 * "UPDATE program_permission_letters SET permission_letter_name=:permissionLetterName, permission_letter_path=:permissionLetterPath"
	 * +
	 * ", uploaded_by=:uploadedBy WHERE program_id=:programId AND permission_letter_id=:permissionLetterId"
	 * , parameterSource); }
	 * 
	 * }
	 */
	...
	public void saveProgramPermissionLetterDetails(ProgramPermissionLetterdetails programPermissionLetterdetails) {
		BeanPropertySqlParameterSource parameterSource = new BeanPropertySqlParameterSource(
				programPermissionLetterdetails);
		if (0 == programPermissionLetterdetails.getPermissionLetterId()) {
			int permissionLetterId = this.jdbcTemplate.query(
					"SELECT permission_letter_id FROM program_permission_letters WHERE program_id=? AND permission_letter_name=? ",
					new Object[] { programPermissionLetterdetails.getProgramId(),
		...
		if (0 == programPermissionLetterdetails.getPermissionLetterId()) {
			...
			this.namedParameterJdbcTemplate.update(
					"UPDATE program_permission_letters SET permission_letter_name=:permissionLetterName, permission_letter_path=:permissionLetterPath"
							...
							+ ", prms_gvr_email_id=:prmsGvrEmailId WHERE program_id=:programId AND permission_letter_id=:permissionLetterId",
							parameterSource);
		}
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * getListOfPermissionLetters(int)
	 */
	@Override
	...
	/**
	 * This method is used to get the program details using email,role of logged
	 * in user and auto generated event Id.
	 * 
	 * @param emailList
	 *            emails associated with Abhyasi Id for the logged in person.
	 * @param userRole,
	 *            role of the logged in user.
	 * @param agEventId,
	 *            auto generated event Id for a particular event.
	 * @return program details based on emails,role of log in user and auto
	 *         generated event Id.
	 */
	@Override
	public Program getProgramByEmailAndRole(List<String> emailList, String userRole, String agEventId) {
		...
		try {
			...
			program = this.namedParameterJdbcTemplate.queryForObject(
					"SELECT DISTINCT p.* " + " FROM program p LEFT JOIN program_coordinators pc"
							+ " ON p.program_id = pc.program_id " + " WHERE "
							+ (whereCondition.length() > 0 ? whereCondition + " AND " : "")
							+ " p.auto_generated_event_id=:auto_generated_event_id ",
							params, BeanPropertyRowMapper.newInstance(Program.class));
	...
	/**
	 * This method is used to get all the program Id and auto generated event
	 * Id's for the logged in user have conducted.
	 * 
	 * @param emailList
	 *            List of emails associated witha abhyasi id for the logged in
	 *            user.
	 * @param userRole,
	 *            role of the logged in user.
	...
	public LinkedHashMap<Integer, String> getListOfProgramIdsByEmail(List<String> emailList, String userRole) {
		...
		try {
			programIds = this.jdbcTemplate.query(
					"SELECT p.program_id,p.auto_generated_event_id  "
							+ " FROM program p LEFT JOIN program_coordinators pc " + " ON p.program_id = pc.program_id "
							+ (whereCondition.length() > 0 ? " WHERE " + whereCondition : ""),
			...
			LOGGER.error(
					"EXCEPTION : while fetching events for the logged in user with email id " + emailList.toString());
		}
		return programIds;
	}*/
	@Override
	public LinkedHashMap<Integer, String> getListOfProgramIdsByEmail(List<String> emailList, String userRole, String coordinatorType, List<String> mysrcmCenters) {
		...
		StringBuilder baseQuery = new StringBuilder("SELECT p.program_id,p.auto_generated_event_id "
				+ " FROM program p LEFT JOIN program_coordinators pc " + " ON p.program_id = pc.program_id ");

		if (!coordinatorType.equalsIgnoreCase(CoordinatorPosition.PRESIDENT.getPositionType())) { 
			...
			if(coordinatorType.equalsIgnoreCase(CoordinatorPosition.COUNTRY_COORDINATOR.getPositionType())){

				params.put("eventCountry",PMPConstants.COUNTRY_INDIA);
				baseQuery.append(" WHERE (p.event_country=:eventCountry OR (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + ")))");

			}else if(coordinatorType.equalsIgnoreCase(CoordinatorPosition.ZONE_COORDINATOR.getPositionType()) 
					|| coordinatorType.equalsIgnoreCase(CoordinatorPosition.CENTER_COORDINATOR.getPositionType())){
				...
				params.put("eventCountry",PMPConstants.COUNTRY_INDIA);
				baseQuery.append(" WHERE ((p.program_center IN (:centers) AND p.event_country=:eventCountry) OR (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + "))) ");
				...
				if (!userRole.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN)) {
					...
					if (userRole.equalsIgnoreCase(PMPConstants.LOGIN_ROLE_ADMIN)) {
						whereCondition.append(" (p.program_channel NOT REGEXP ('G-Connect|G Connect|GConnect|G-Conect|G - Connect|G.CONNECT|G -CONNECT|G- connect|G-Connet|G  Connect')"
								+ " OR (p.coordinator_email IN( "+ emailString+ ") OR pc.email IN("+ emailString+ "))) ");
					} else {
	...
	private void updateCoordinatorHistory(int programId, CoordinatorHistory updatedcrdntrHstry) {
		...
		try {
			oldcrdntrHstry = this.namedParameterJdbcTemplate.queryForObject(
					"SELECT id,program_id,coordinator_name,coordinator_email,"
							...
							+ " WHERE program_id=:programId ORDER BY assigned_time DESC LIMIT 1",
							params, BeanPropertyRowMapper.newInstance(CoordinatorHistory.class));
		} catch (Exception ex) {
		...
		if (null == oldcrdntrHstry) {
			...
			if (!updatedcrdntrHstry.getCoordinatorEmail().equals(oldcrdntrHstry.getCoordinatorEmail())) {
				...
				this.namedParameterJdbcTemplate.update(
						"UPDATE coordinator_history SET " + "removal_time = CURRENT_TIMESTAMP " + "WHERE id=:id",
						parameterSource);
				saveCoordinatorHistory(updatedcrdntrHstry);
			}
		}
	...
	public List<Integer> getProgramIdsForSQSPush() {
		return this.jdbcTemplate.queryForList(
				"SELECT DISTINCT(pr.program_id)" + " FROM program p,participant pr"
	 ...
	 * 
	 * @see org.srcm.heartfulness.repository.ProgramRepository#
	 * saveProgramTestimonialDetails(org.srcm.heartfulness.model.
	 * ProgramTestimonialDetails)
	 */
	@Override
	public void saveProgramTestimonialDetails(ProgramTestimonialDetails testimonialDetails) {
		...
		if (0 == testimonialDetails.getTestimonialId()) {
			...
			this.namedParameterJdbcTemplate.update(
					"UPDATE program_testimonials SET testimonial_name=:testimonialName, permission_letter_path=:testimonialPath"
	...
	public void updateProgramStatus(Program program, String programStatus) {
		BeanPropertySqlParameterSource parameterSource = new BeanPropertySqlParameterSource(program);
		this.namedParameterJdbcTemplate.update(
				"UPDATE program set program_status =:programStatus where program_id=:programId ", parameterSource);
	}

	@Override
	public Program getProgramByEmailAndRole(List<String> emailList, String userRole, String agEventId,
			String coordinatorType, List<String> mysrcmCenters) {

		Program program = null;
		Map<String, Object> params = new HashMap<>();
		params.put("autoGeneratedEventId", agEventId);
		StringBuilder whereCondition = new StringBuilder("");
		StringBuilder emailString = new StringBuilder("");

		StringBuilder baseQuery = new StringBuilder("SELECT DISTINCT p.* "
				+ "FROM program p LEFT JOIN program_coordinators pc ON p.program_id = pc.program_id "
				+" WHERE p.auto_generated_event_id=:autoGeneratedEventId ");
		...
		if(!coordinatorType.equalsIgnoreCase(CoordinatorPosition.PRESIDENT.getPositionType())){
			...
			if(coordinatorType.equalsIgnoreCase(CoordinatorPosition.COUNTRY_COORDINATOR.getPositionType())){
				...
				params.put("eventCountry",PMPConstants.COUNTRY_INDIA);
				baseQuery.append("  AND (p.event_country=:eventCountry OR (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + ")))");
				...
				params.put("eventCountry",PMPConstants.COUNTRY_INDIA);
				baseQuery.append(" AND ((p.program_center IN (:centers) AND p.event_country=:eventCountry) OR (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + ")))");
				...
				if (!userRole.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN)) {
					...
					if(userRole.equalsIgnoreCase(PMPConstants.LOGIN_ROLE_ADMIN)){
						...
						whereCondition.append(" AND (p.program_channel NOT REGEXP ('G-Connect|G Connect|GConnect|G-Conect|G - Connect|G.CONNECT|G -CONNECT|G- connect|G-Connet|G  Connect')"
								+	" OR (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + "))) " );
					}else{
						whereCondition.append(" AND (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + ")) ");
					}
		...
		try{
			program = this.namedParameterJdbcTemplate.queryForObject(baseQuery.toString() + 
					(whereCondition.length() > 0 ?  whereCondition : " "), params, BeanPropertyRowMapper.newInstance(Program.class));
			program.setIsReadOnly(CoordinatorAccessControlConstants.IS_READ_ONLY_FALSE);

		} catch(Exception ex){
			program = this.namedParameterJdbcTemplate.queryForObject("SELECT DISTINCT p.* "
					+ "FROM program p LEFT JOIN program_coordinators pc ON p.program_id = pc.program_id "
					+" WHERE p.auto_generated_event_id=:autoGeneratedEventId ", params, BeanPropertyRowMapper.newInstance(Program.class));
			program.setIsReadOnly(CoordinatorAccessControlConstants.IS_READ_ONLY_TRUE);
		}
	...
	@Override
	public int getProgramCountForLogInCoordinator(List<String> emailList, String role, String coordinatorType,List<String> mysrcmCenters) {
		...
		StringBuilder baseQuery = new StringBuilder("SELECT count(DISTINCT p.program_id ) "
		...
		if(!coordinatorType.equalsIgnoreCase(CoordinatorPosition.PRESIDENT.getPositionType())){
			...
			if(coordinatorType.equalsIgnoreCase(CoordinatorPosition.COUNTRY_COORDINATOR.getPositionType())){
				...
				params.put("eventCountry",PMPConstants.COUNTRY_INDIA);
				baseQuery.append(" WHERE ((p.event_country=:eventCountry AND p.program_center IN (:centers)) OR (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + ")))");
		...
		return  this.namedParameterJdbcTemplate.queryForObject(baseQuery.toString() + (whereCondition.length() > 0 ? " WHERE " + whereCondition : ""), params, Integer.class);
	...
	public List<Program> getEventsByEmailAndRole(List<String> emailList, String role, int offset, int pageSize,String coordinatorType, List<String> mysrcmCenters) {
		...
		StringBuilder baseQuery = new StringBuilder("SELECT DISTINCT p.program_id,p.program_channel,p.program_start_date,"
				+ "p.program_end_date,p.coordinator_name,p.coordinator_email,p.coordinator_mobile,"
				+ "p.event_place,p.event_city,p.event_state,p.event_country,p.organization_department,"
				+ "p.organization_name,p.organization_web_site,p.organization_contact_name,"
				+ "p.organization_contact_email,p.organization_contact_mobile,p.preceptor_name,"
				...
				+ "p.batch_description,p.program_channel_type,p.program_address,p.program_district,p.program_zone,"
				+ "p.program_center,p.organization_contact_name,p.organization_contact_designation,p.organization_contact_email,"
				+ "p.organization_contact_mobile,p.coordinator_abhyasi_id,p.coordinator_name,p.coordinator_email,p.coordinator_mobile,p.program_name,p.program_status "
				+ "FROM program p LEFT JOIN program_coordinators pc " + " ON p.program_id = pc.program_id ");
		...
		return programs = this.namedParameterJdbcTemplate.query(baseQuery.toString()
				+ (whereCondition.length() > 0 ? " WHERE " + whereCondition : "")
				+ " ORDER BY p.program_start_date DESC" + (limitCondition.length() > 0 ? limitCondition : ""),params,BeanPropertyRowMapper.newInstance(Program.class));

	}

	@Override
	public int getPgrmCountBySrchParamsForLogInCoordinator(SearchRequest searchRequest, List<String> emailList,
			String role, String coordinatorType, List<String> mysrcmCenters) {
			...
			if(coordinatorType.equalsIgnoreCase(CoordinatorPosition.COUNTRY_COORDINATOR.getPositionType())){
				...
				if (!role.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN)) {
					...
					if(role.equalsIgnoreCase(PMPConstants.LOGIN_ROLE_ADMIN)){
						...
						whereCondition.append(" WHERE (p.program_channel NOT REGEXP ('G-Connect|G Connect|GConnect|G-Conect|G - Connect|G.CONNECT|G -CONNECT|G- connect|G-Connet|G  Connect')"
								+	" OR (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + "))) " );
					}else{
						whereCondition.append(" WHERE (p.coordinator_email IN( " + emailString +") OR pc.email IN("+ emailString + ")) ");
					}
				}
			}
		}
		...
		if (!("ALL".equals(searchRequest.getSearchField())) && null != searchRequest.getSearchField() && !searchRequest.getSearchField().isEmpty()) {
			...
			if (null != searchRequest.getSearchText() && !searchRequest.getSearchText().isEmpty()){
				if(role.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN) || coordinatorType.equalsIgnoreCase(CoordinatorPosition.PRESIDENT.getPositionType())) {
					whereCondition.append(" WHERE p." + searchRequest.getSearchField() + " LIKE '%" + searchRequest.getSearchText() + "%'" );
				}else{
					whereCondition.append(" AND p." + searchRequest.getSearchField() + " LIKE '%" + searchRequest.getSearchText() + "%'" );
				}
			}
		}
	...
	@Override
	public List<Program> searchEventsWithUserRoleAndEmailId(SearchRequest searchRequest, List<String> emailList,
			String role, int offset, String coordinatorType, List<String> mysrcmCenters) {
		...
		StringBuilder baseQuery = new StringBuilder("SELECT DISTINCT p.program_id,p.auto_generated_event_id,p.program_channel, "
				+ " p.program_name,p.program_start_date,p.program_end_date, "
				+ " p.coordinator_name,p.coordinator_email,p.coordinator_mobile,"
				+ " p.event_place,p.event_city,p.event_state,p.event_country,p.organization_name,"
				+ " p.organization_department,p.preceptor_name,p.preceptor_id_card_number,p.jira_issue_number,"
		...
		programs = this.namedParameterJdbcTemplate.query(baseQuery.toString() + (whereCondition.length() > 0 ? whereCondition : "")
				+ (orderBy.length() > 0 ? " ORDER BY " + orderBy : " ORDER BY p.program_start_date DESC ")
				+ " LIMIT " + offset + "," + searchRequest.getPageSize(),
				params, BeanPropertyRowMapper.newInstance(Program.class));
		return programs;
	...
	public ArrayList<String> getProgramIdAndMaxSessionDate(String autoGeneratedEventId) {
		...
		ArrayList<String> date = this.jdbcTemplate.query("SELECT  p.program_id, max(s.session_date) FROM program p "
				+"LEFT OUTER JOIN session_details s "
				+"ON p.program_id = s.program_id  WHERE p.auto_generated_event_id = ?",new Object[]{autoGeneratedEventId}, new ResultSetExtractor<ArrayList<String>>(){
	...
	@Override
	public Program getProgramByEmailAndRoleForParticipant(List<String> emailList, String userRole, String agEventId) {
		...
		try {
			/*(null != userRole && userRole.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN)) || whereCondition.length() > 0)*/
			if ((null != userRole && (userRole.equalsIgnoreCase(PMPConstants.LOGIN_GCONNECT_ADMIN) || userRole.equalsIgnoreCase(PMPConstants.LOGIN_ROLE_PRESIDENT))) || whereCondition.length() > 0 ){
				program = this.namedParameterJdbcTemplate.queryForObject(
						baseQuery.toString() + (whereCondition.length() > 0 ? whereCondition : " "), params,
						BeanPropertyRowMapper.newInstance(Program.class));
				program.setIsReadOnly(CoordinatorAccessControlConstants.IS_READ_ONLY_FALSE);
			} else {
				program = this.namedParameterJdbcTemplate.queryForObject("SELECT DISTINCT p.* "
						+ "FROM program p LEFT JOIN program_coordinators pc ON p.program_id = pc.program_id "
						+ " WHERE p.auto_generated_event_id=:autoGeneratedEventId ", params,
						BeanPropertyRowMapper.newInstance(Program.class));
				program.setIsReadOnly(CoordinatorAccessControlConstants.IS_READ_ONLY_TRUE);
			}
